...
# السماح للتطبيق بإدارة توجيه المكسيك/VPN من اللوحة (sudo بلا كلمة مرور لسكربتات محددة)
chmod +x "$APP_DIR/deploy/vpn-apply.sh" "$APP_DIR/deploy/vpngate-apply.sh" 2>/dev/null || true
echo "$SERVICE_USER ALL=(root) NOPASSWD: $APP_DIR/deploy/vpn-apply.sh, $APP_DIR/deploy/vpngate-apply.sh" > /etc/sudoers.d/iptvpro
chmod 440 /etc/sudoers.d/iptvpro
DEBIAN_FRONTEND=noninteractive apt-get install -y wireguard openvpn >/dev/null 2>&1 || true
