(function () {
  ...
  var UI_VERSION = "cr6608-smartap-v29-mobile-r1-25.12.5";
  ...
  var L = {
    ar: {
      ...
      passwordHint: "admin", login: "دخول", logout: "خروج",
      loginWait: "جاري التحقق من بيانات الدخول...", loginBad: "فشل تسجيل الدخول. استخدم root / admin.",
      loginRateLimited: "محاولات كثيرة. انتظر دقيقة ثم استخدم root / admin.",
    ...
    en: {
      ...
      passwordHint: "admin", login: "Sign in", logout: "Logout",
      loginWait: "Checking credentials...", loginBad: "Login failed. Use root / admin.",
      loginRateLimited: "Too many attempts. Wait one minute, then use root / admin.",
  ...
  async function ensureLuciSession() {
    ...
    var res = await fetch("/cgi-bin/dashluci", {
      method: "POST",
    ...
    if (!res.ok || !data.ok) throw new Error(data.error || "LuCI session failed");
    return true;
  }
  async function revokeServerSessions() {
    var res = await fetch("/cgi-bin/dashlogout", {
      method: "POST",
      credentials: "same-origin",
      cache: "no-store",
  ...
  async function login(username, password) {
    ...
    try {
      ...
      var data = await res.json().catch(function () { return {}; });
      if (!res.ok || !data.ok) {
        throw new Error(res.status === 429 ? tr("loginRateLimited") : tr("loginBad"));
      }
      ...
      state.session = "";
      sessionStorage.removeItem(LS + "session");
      showLogin(e.message || tr("loginBad"), true);
    }
  }
  async function validateSession() {
    if (!state.session) return false;
    try {
      var res = await fetch(authUrl(API), { cache: "no-store", headers:authHeaders() });
  ...
  function renderWifi(data) {
    ...
    var body = w.length ? '<div class="grid two">' + w.map(function (x) {
      ...
      var txShown = finite(txApplied) ? txApplied : (finite(txReq) ? txReq : 38);
      var powerRow = '<div><span>' + (state.lang === "ar" ? "TX المطبق" : "Applied TX") + '</span><b class="latin" style="color:var(--excellent)">' + fmt(txShown, 0) + ' dBm</b></div>' +
  ...
  function renderIsolationControl(data) {
    ...
    var ports = ["lan1", "lan2", "lan3"].map(function (key, idx) {
      ...
      return '<section class="dsa-port-row" data-dsa-port="' + key + '"><div class="dsa-port-head"><div class="icon">' + icon("net") + '</div><div><strong>LAN' + (idx + 1) + '</strong><span>' + esc(status.value || "") + '</span><small>' + esc(status.hint || "") + '</small></div></div><div class="dsa-port-controls">' + fields.map(fieldHtml).join("") + '</div></section>';
    }).join("");
    var ar = state.lang === "ar";
    ...
    if (type === "number" && f.options) {
      ...
      if (/^txpower(?:_radio[01])?$/.test(f.name)) {
        var presets = [5,10,15,20,23,26,30,35,38].map(function (v) {
          var label = v === 38 ? "Max / Turbo 38" : String(v);
  ...
  function renderWizardControl(section, data) {
    ...
    return cards +
      ...
      '</div>' +
      '<p class="mode-hint">بعد التطبيق اختبر الاتصال ثم اضغط احتفظ بالتغييرات خلال 120 ثانية؛ وإلا يرجع الجهاز تلقائياً. طاقة البث قابلة للاختيار من 1 إلى 38 dBm.</p>' +
...
return H.card(A?'أبعد العملاء':'Distance Leaderboard',h,String(L.length)+(A?' جهاز':' clients'),'device')}},
  ...
  function renderOverviewLite(data, rates) {
    ...
    var radios = [w24, w5].filter(Boolean).map(function (w) {
      var p = w.txpower || {}, applied = num(p.applied_dbm), requested = num(p.requested_dbm);
  ...
  async function loadData(forceFull) {
    ...
    try {
      ...
      var full = forceFull === true || fullDue;
      var res = await fetch(authUrl(full ? API : API + "?lite=1"), { cache:"no-store", headers:authHeaders(), signal: ctrl ? ctrl.signal : undefined });
  ...
  async function speedTest() {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      var r = await fetch(authUrl(API + "?speed=" + i), { cache:"no-store", headers:authHeaders() });
  ...
  async function loadControl(section, actionName, params, button) {
    ...
    try {
      ...
      var fetchOptions = { cache:"no-store", headers:authHeaders() };
      if (actionName) {
        url = CTL;
        fetchOptions = {
          method:"POST",
  ...
  function bindDynamic() {
    ...
    Array.prototype.forEach.call(document.querySelectorAll(".txpower-control"), function (box) {
      ...
      number.oninput = function () {
        ...
        if (isFinite(value) && value >= 1 && value <= 38) range.value = String(value);
    ...
    Array.prototype.forEach.call(document.querySelectorAll(".dev-action"), function (b) {
      b.onclick = async function () {
        ...
        if (act === "__limit") { // bandwidth-hog "limit" -> ask Mbps, set per-client QoS
          ...
          try {
            var rl = await fetch(CTL, { method:"POST", cache:"no-store", headers:authHeaders({ "Content-Type":"application/x-www-form-urlencoded" }),
