...
| `htmode HE80` (5 GHz) / `HE40` (2.4) | 80 MHz default; **160 MHz is a selectable operator choice** | The MT7915 silicon and the mt76 driver both fully support HE160 — the same 2×2 chip runs 160 MHz in AX3000 SKUs. The vendor rates these boxes **AX1800**, i.e. the stock default mode is 80 MHz (1201 Mbps), so **80 MHz stays the shipped default** here too. What changed: the firmware no longer force-downgrades a 160 MHz request — set `htmode5 HE160` in Quick Settings/LuCI/uci to opt in. Made safe, not just permissive: 160 MHz on a 2×2 7915 is the flaky width upstream (mt76#748) and always spans DFS sub-channels (ch36 → 36-64), so it needs a full CAC; Safe Apply's rollback window auto-extends to 720 s for HE160 so a valid bring-up isn't reverted mid-CAC, and if the radio can't bring 160 up it rolls back to the prior width automatically. Note 160 MHz is a **throughput** choice — it *reduces* range (opposite of `smartap-range-mode`). All 5 GHz **channels** stay selectable exactly as before |
