(function () {
  ...
  var UI_VERSION = "cr6608-smartap-lab38-r11-25.12.5";
  ...
  var state = {
    ...
    interval: Number(localStorage.getItem(LS + "interval") || 3),
    timer: 0,
    latest: null,
  ...
  var L = {
    ar: {
      ...
      quickNote: "اختر الوضع وعبّئ الحقول ثم اضغط حفظ وتطبيق. اختبر الاتصال ثم اضغط احتفظ بالتغييرات خلال 120 ثانية، وإلا يرجع الجهاز تلقائياً للإعداد السابق.",
      isolationHint: "حمايات وتحكم بالمنافذ — كل خيار مشروح تحته.",
      isolationTitle: "الحماية والمنافذ — دليل مبسّط",
      isolationNote: "٣ أقسام: الحمايات العامة، عزل الواي فاي، ومنافذ الكيبل. بعد التطبيق اختبر الاتصال ثم ثبّت العملية المطابقة خلال 120 ثانية، وإلا يتم الرجوع تلقائياً.",
      ...
      passwordHint: "كلمة مرور الواجهة: admin", login: "دخول", logout: "خروج",
      loginWait: "جاري التحقق من بيانات الدخول...", loginBad: "فشل تسجيل الدخول. استخدم root وكلمة مرور الواجهة admin.",
      loginRateLimited: "محاولات كثيرة. انتظر دقيقة ثم استخدم كلمة مرور الواجهة admin.",
      loginUnavailable: "تعذر الوصول إلى خدمة تسجيل الدخول. تحقق من اتصالك بالراوتر ثم حاول مرة أخرى.",
      openWrtAuthFailed: "تعذر فتح إعدادات OpenWrt بأمان. أعد تسجيل الدخول ثم حاول مرة أخرى.",
    ...
    en: {
      ...
      quickNote: "Pick a mode, then Save & Apply. Test connectivity and explicitly keep the matching transaction within 120 seconds, otherwise it rolls back automatically.",
      isolationHint: "Protections & port control — each option explained below it.",
      isolationTitle: "Protection & Ports — simple guide",
      isolationNote: "General protection, Wi-Fi isolation and wired-port policy. After Apply, test connectivity and explicitly keep the matching transaction within 120 seconds or it rolls back automatically.",
      ...
      passwordHint: "Interface password: admin", login: "Sign in", logout: "Logout",
      loginWait: "Checking credentials...", loginBad: "Login failed. Use root with the interface password admin.",
      loginRateLimited: "Too many attempts. Wait one minute, then use the interface password admin.",
      loginUnavailable: "The sign-in service did not respond. Check the router connection and try again.",
      openWrtAuthFailed: "Could not securely open OpenWrt settings. Sign in again and retry.",
  ...
  async function syncBrowserTime() {
    ...
    var killer = ctrl ? setTimeout(function () { ctrl.abort(); }, 5000) : null;
    try {
      await fetch(CTL, {
        method:"POST",
  ...
  async function login(username, password) {
    ...
    try {
      ...
      showDashboard();
      syncBrowserTime();
      // A temporary LuCI bridge failure must not turn valid Smart AP credentials
      // into a false password error. The OpenWrt button retries this exchange.
      try {
        await ensureLuciSession();
      } catch (luciError) {
        toast(tr("openWrtAuthFailed"));
      }
    } catch (e) {
      if (state.session) {
  ...
  async function validateSession() {
    ...
    try {
      var res = await fetchWithTimeout(authUrl(API), { credentials:"same-origin", cache:"no-store", headers:authHeaders() }, 8000);
  ...
  function showSection(id) {
    ...
    if (state.latest && /^(overview|network|devices|wifi|insights|system|actions)$/.test(id)) {
      renderLiveSection(id, state.latest, trafficRates(state.latest));
    }
  ...
  async function scanLan() {
    ...
    try {
      var r = await fetch(CTL, { method: "POST", credentials:"same-origin", cache: "no-store",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
  ...
  function renderWifi(data) {
    ...
    var body = w.length ? '<div class="grid two">' + w.map(function (x) {
      ...
      var sta = (x.stations || []).map(function (s) {
        ...
        return '<div class="kv"><div><span class="latin">' + esc(s.ip || s.mac || tr("unavailable")) + steer + '</span><b class="latin">' + (finite(ss) ? ss + " dBm " : "") + proximity(ss) + '</b></div>' + bar(finite(ss) ? signalPct("rssi", ss) : 0, 100, qq.color) +
          '<div class="cli-tags">' + effStr + retryStr + failStr + '</div>' + rfStr + metaStr + trafficStr + trendStr + '</div>';
      ...
      // Owner preference: show the configured TX power only. The driver-reported
      // "applied" figure and the channel ceiling are no longer surfaced here — they
      // are still available truthfully via `cr6608-txpower-verify`,
      // `logread | grep TXTRACE` and the dashapi2 payload, which are unchanged.
      var powerRow = '<div><span>' + (state.lang === "ar" ? "قوة الإرسال" : "TX power") + '</span><b class="latin">' + (finite(txReq) ? fmt(txReq, 0) + " dBm" : tr("unavailable")) + '</b></div>' +
      ...
      return card(esc(x.ssid || x.iface), '<div class="kv"><div><span>Band</span><b>' + esc(x.band || "") + '</b></div><div><span>' + tr("channel") + '</span><b>' + esc(x.channel || "") + '</b></div><div><span>Mode</span><b class="latin">' + esc(x.htmode || "") + '</b></div><div><span>Clients</span><b>' + (x.clients || 0) + '</b></div><div><span>' + rssiLabel + '</span><b class="latin" style="color:' + q.color + '">' + (finite(sig) ? fmt(sig, 0) + " dBm " + proximity(sig) : tr("unavailable")) + '</b></div>' + powerRow + busyRow + '</div><h4>Clients · ' + tr("linkRate") + '</h4>' + sta, esc(x.hw_modes || ""), "wifi");
    ...
    return sectionHead("WiFi AX / AC / N", state.lang === "ar" ? "TX Power منفصل عن RSSI العملاء ومعدل الربط" : "TX power, client RSSI and link rate are separate measurements", "offline") +
  ...
  function renderIsolationControl(data) {
    ...
    var guardCards = cards.filter(function (c) { return !/^LAN[12]$/.test(c.label) && !/واي فاي|Wi-Fi/i.test(c.label); });
    var wifiCards = cards.filter(function (c) { return /واي فاي|Wi-Fi/i.test(c.label); });
    var portCards = cards.filter(function (c) { return /^LAN[12]$/.test(c.label); });
    ...
    var ports = ["lan1", "lan2"].map(function (key, idx) {
    ...
    var ar = state.lang === "ar";
    var tabs = '<div class="isolation-tabs" role="tablist">' +
      '<button data-isolation-tab="guard">' + (ar ? "الحماية" : "Protection") + '</button>' +
      '<button data-isolation-tab="wifi">' + (ar ? "عزل Wi-Fi" : "Wi-Fi isolation") + '</button>' +
      '<button class="active" data-isolation-tab="ports">' + (ar ? "المنافذ" : "Ports") + '</button></div>';
    return tabs +
      '<section class="isolation-panel" data-isolation-panel="guard" hidden>' + isolationStatusCards(guardCards) + '<div class="ctl-form">' + guardFields.map(fieldHtml).join("") + '</div></section>' +
      '<section class="isolation-panel" data-isolation-panel="wifi" hidden>' + isolationStatusCards(wifiCards) + '<div class="ctl-form">' + wifiFields.map(fieldHtml).join("") + '</div></section>' +
      '<section class="isolation-panel" data-isolation-panel="ports">' + topology + '<div class="dsa-port-list">' + ports + '</div></section>' +
  ...
  function syncDsaPortRows() {
    Array.prototype.forEach.call(document.querySelectorAll("[data-dsa-port]"), function (row) {
      ...
      [mode, vlan, isolate, enabled].forEach(function (el) {
        if (!el) return;
        el.onchange = syncDsaPortRows;
        if (el === vlan) el.oninput = syncDsaPortRows;
      });
    });
  }
  ...
  function renderActions() {
    ...
    var ar = state.lang === "ar";
    // Firmware flashing, self-contained in the Smart AP page: upload via cgi-io
    // then run sysupgrade. Does not depend on the LuCI flash page at all.
    var fw = '<div class="card" style="margin-top:14px">' +
      '<h3 style="margin:0 0 6px">' + esc(ar ? "تحديث النظام (رفع ملف)" : "Firmware upgrade (upload)") + '</h3>' +
      '<p style="margin:0 0 10px;color:var(--muted);font-size:13px">' +
      esc(ar ? "اختر ملف sysupgrade.bin ثم اضغط رفع وتثبيت. لا تفصل الكهرباء أثناء التثبيت."
             : "Choose a sysupgrade.bin image, then upload and flash. Do not power off during flashing.") + '</p>' +
      '<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">' +
      '<input type="file" id="fwFile" accept=".bin" style="max-width:100%">' +
      '<label class="btn" style="display:inline-flex;align-items:center;gap:6px">' +
      '<input type="checkbox" id="fwKeep" checked> ' + esc(ar ? "حفظ الإعدادات" : "Keep settings") + '</label>' +
      '<button class="btn primary" id="fwFlashBtn">' + esc(ar ? "رفع وتثبيت" : "Upload & flash") + '</button>' +
      '</div>' +
      '<div id="fwMsg" style="margin-top:10px;font-size:13px;color:var(--muted)"></div>' +
      '</div>';
    return sectionHead(tr("actions"), tr("actionsHint"), tr("safeAction")) +
      '<div class="actions">' + actions.map(function (a) { return '<button class="action" data-action="' + a[0] + '"><strong>' + esc(a[1]) + '</strong><span>' + esc(a[2]) + '</span></button>'; }).join("") + '</div>' +
      fw + renderEvents(state.latest || {});
  }

  // Wire the firmware upload card. Called after the Actions section renders.
  function bindFirmwareUpload() {
    ...
    btn.onclick = async function () {
      ...
      if (!f) return say(ar ? "اختر ملف التحديث أولاً." : "Choose an image file first.", true);
      if (!/\.bin$/i.test(f.name)) return say(ar ? "الملف يجب أن ينتهي بـ .bin" : "The file must end with .bin", true);
      if (!window.confirm(ar ? ("تثبيت " + f.name + " الآن؟ لا تفصل الكهرباء.") : ("Flash " + f.name + " now? Do not power off."))) return;
      btn.disabled = true;
      say(ar ? "جاري رفع الملف..." : "Uploading image...");
      try {
        // Send the raw image body to our own cookie-authenticated CGI. The old path
        // posted to cgi-io with sessionid="cookie" (a UI marker, not a ubus session id),
        // which cgi-io always rejected with 403, so flashing never started. dashupload
        ...
        btn.disabled = false;
        say((ar ? "فشل: " : "Failed: ") + e.message, true);
      }
    };
  }
...
{key:"load_avg",ar:"متوسط الحمل",en:"Load Average",cat:"System & Health",fn:function(d,H){var l=d.load||[];if(!l.length)return H.card(H.lang==="ar"?"متوسط الحمل":"Load","<div class='empty'>—</div>",null,"cpu");var cores=(d.cpu&&d.cpu.cores)||1,labs=["1m","5m","15m"];var r=l.slice(0,3).map(function(v,i){var la=(H.num(v)||0),p=H.clamp(la/cores*100,0,100),col=p<60?"var(--excellent)":p<90?"var(--mid)":"var(--weak)";return "<div style='margin:6px 0'><div style='display:flex;justify-content:space-between;font-size:12px'><span>"+labs[i]+"</span><span class='latin'>"+H.fmt(la,2)+"</span></div>"+H.bar(p,100,col)+"</div>";}).join("");return H.card(H.lang==="ar"?"متوسط الحمل":"Load Average",r,cores+" cores","cpu");}},
...
return H.card(A?'أبعد العملاء':'Distance Leaderboard',h,String(L.length)+(A?' جهاز':' clients'),'device')}},
  ...
  PRO_FEATURES.push({key:"mimo_matrix",ar:"مصفوفة AX/AC/N",en:"AX/AC/N Matrix",cat:"Automation & UX",fn:function(d,H){
    ...
    h+="</tbody></table></div><p class='muted' style='margin-top:6px;font-size:11px'>"+(A?"cfg = مهيّأ في النظام، وليس دليلاً على جدولة هوائية فعلية. قياس OFDMA وMU‑MIMO يحتاج عدة عملاء متوافقين.":"cfg = configured, not proof of over-air scheduling. OFDMA and MU‑MIMO measurement needs multiple compatible clients.")+"</p>";
  ...
  function renderProInsights(data) {
    ...
    var ifaceFaults = (data.interfaces || []).reduce(function (total, row) {
      return total + (num(row.rx_errors) || 0) + (num(row.tx_errors) || 0) +
        (num(row.rx_dropped) || 0) + (num(row.tx_dropped) || 0);
    }, 0);
    ...
    out += '<div class="insight-summary">' +
      ...
      summary(state.lang === "ar" ? "أخطاء المنافذ" : "Port faults", String(ifaceFaults), state.lang === "ar" ? "أخطاء وإسقاطات" : "errors and drops", ifaceFaults ? "warn" : "ok") +
      '</div>';
  ...
  function renderOverviewLite(data, rates) {
    ...
    var throughput = '<div class="grid two"><div class="traffic-box"><span>RX</span><b class="latin">' + bps(rates.rx) + '</b></div><div class="traffic-box"><span>TX</span><b class="latin">' + bps(rates.tx) + '</b></div></div>';
    ...
    // Show the operator's own record (customer name / phone / device) explicitly, not
    ...
    return sectionHead(tr("overview"), tr("subtitle"), nowTime()) + '<div class="grid two">' +
      card(tr("networkTitle"), throughput, "live", "net") +
      card(state.lang === "ar" ? "الراديو والبث" : "Radios & TX", radios || '<div class="empty">—</div>', "iw", "wifi") +
      card(state.lang === "ar" ? "حالة الاتصال" : "Connection status", status, data.ok ? tr("online") : tr("offline"), "bolt") +
  ...
  function renderLiveSection(id, data, rates) {
  ...
  function render(data) {
    ...
    renderLiveSection(document.body.dataset.activeSection || "overview", data, rates);
    setTimeout(loadActiveControl, 0);
    saveHistories();
  }
  async function loadData(forceFull) {
    ...
    try {
      ...
      var bootstrapLite = !state.latest && forceFull !== true;
      var fullDue = !!state.latest && Date.now() - state.lastFullAt >= (detailed ? 15000 : 60000);
      var full = forceFull === true || (!bootstrapLite && fullDue);
      ...
      else if (!data.lite) state.lastFullAt = Date.now();
  ...
  async function action(name) {
    ...
    try {
      ...
      if (j.ok && (name === "wifi_radio0" || name === "wifi_radio1"))
        setTimeout(function () { loadData(true); }, 4500);
    } catch (e) { toast(e.message); }
  }
  ...
  async function loadControl(section, actionName, params, button) {
    ...
    try {
      ...
      if (data.summary) toast(data.summary);
      if (actionName === "save_identity" && data.ok) setTimeout(function () { loadData(true); }, 100);
      bindDynamic();
    } catch (e) {
  ...
  function bindDynamic() {
    Array.prototype.forEach.call(document.querySelectorAll("[data-insight-category]"), function (button) {
    ...
    Array.prototype.forEach.call(document.querySelectorAll(".txpower-control"), function (box) {
    ...
    Array.prototype.forEach.call(document.querySelectorAll("[data-tx-preset]"), function (button) {
      button.onclick = function () {
    ...
    Array.prototype.forEach.call(document.querySelectorAll("[data-isolation-tab]"), function (b) {
      b.onclick = function () {
        var key = b.dataset.isolationTab;
        Array.prototype.forEach.call(document.querySelectorAll("[data-isolation-tab]"), function (x) { x.classList.toggle("active", x === b); });
        Array.prototype.forEach.call(document.querySelectorAll("[data-isolation-panel]"), function (p) { p.hidden = p.dataset.isolationPanel !== key; });
      };
    });
    syncDsaPortRows();
    ...
    Array.prototype.forEach.call(document.querySelectorAll(".dev-action"), function (b) {
      b.onclick = async function () {
        ...
        if (act === "__limit") { // bandwidth-hog "limit" -> ask Mbps, set per-client QoS
          ...
          try {
            var rl = await fetch(CTL, { method:"POST", credentials:"same-origin", cache:"no-store", headers:authHeaders({ "Content-Type":"application/x-www-form-urlencoded" }),
  ...
  function startPolling() {
    ...
    clearInterval(state.timer);
    loadData();
    state.timer = setInterval(loadData, Math.max(3, state.interval) * 1000);
  }
  function init() {
    var staleSid = sessionStorage.getItem(LS + "session") || "";
    state.session = staleSid;
    renderChrome();
    setLoginText();
    if ($("loginForm")) {
    ...
    $("intervalSelect").onchange = function () { state.interval = Number(this.value) || 3; localStorage.setItem(LS + "interval", state.interval); startPolling(); };
    ...
    if ($("openWrtBtn")) $("openWrtBtn").onclick = async function () {
      ...
      try {
        ...
        if (e && (e.status === 401 || e.status === 403)) requireLogin(tr("loginBad"));
    ...
    if (staleSid) {
      startupSessionCleanup = validateSession().then(function (valid) {
        if (valid) {
          showDashboard();
          syncBrowserTime();
          return;
        }
        return revokeServerSessions().catch(function () {}).then(function () {
          showLogin(tr("loginBad"), true);
        });
      });
    } else {
      startupSessionCleanup = Promise.resolve();
      showLogin();
    }
  }
