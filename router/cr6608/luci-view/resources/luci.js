...
catch(e){return false;}}});const View=Class.extend({__name__:'LuCI.view',__init__(){const vp=document.getElementById('view');DOM.content(vp,E('div',{'class':'spinning'},_('Loading view…')));const ready=L.loaded?Promise.resolve():new Promise((resolve)=>{document.addEventListener('luci-loaded',resolve,{once:true});});return ready.then(LuCI.prototype.bind(this.load,this)).then(LuCI.prototype.bind(this.render,this)).then(LuCI.prototype.bind(function(nodes){const vp=document.getElementById('view');DOM.content(vp,nodes);DOM.append(vp,this.addFooter());},this)).catch(LuCI.prototype.error);},load(){},render(){},handleSave(ev){const tasks=[];document.getElementById('maincontent').querySelectorAll('.cbi-map').forEach(map=>{tasks.push(DOM.callClassMethod(map,'save'));});return Promise.all(tasks);},handleSaveApply(ev,mode){mode='1';/*owner: every save is FINAL (apply unchecked), no 90s rollback*/return this.handleSave(ev).then(()=>{classes.ui.changes.apply(mode=='0');});},handleReset(ev){const tasks=[];document.getElementById('maincontent').querySelectorAll('.cbi-map').forEach(map=>{tasks.push(DOM.callClassMethod(map,'reset'));});return Promise.all(tasks);},addFooter(){const footer=E([]);const vp=document.getElementById('view');let hasmap=false;let readonly=true;vp.querySelectorAll('.cbi-map').forEach(map=>{const m=DOM.findClassInstance(map);if(m){hasmap=true;if(!m.readonly)
readonly=false;}});if(!hasmap)
readonly=!LuCI.prototype.hasViewPermission();const saveApplyBtn=this.handleSaveApply?E('button',{'class':'btn cbi-button cbi-button-apply important','click':classes.ui.createHandlerFn(this,'handleSaveApply'),'disabled':readonly||null},[_('حفظ نهائي')]):E([]);if(this.handleSaveApply||this.handleSave||this.handleReset){footer.appendChild(E('div',{'class':'cbi-page-actions'},[saveApplyBtn,' ',this.handleReset?E('button',{'class':'cbi-button cbi-button-reset','click':classes.ui.createHandlerFn(this,'handleReset'),'disabled':readonly||null},[_('Reset')]):'']));}
