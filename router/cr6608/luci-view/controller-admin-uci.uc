...
return {
	action_apply_rollback: function() {
		/* Owner rule: EVERY apply is final. Old cached clients still call this
		 * endpoint expecting a rollback session; commit immediately instead and
		 * hand back a dummy token so they finish their normal flow. Nothing is
		 * ever armed in rpcd, so nothing can revert at T+90s. */
		uci_apply(false);

		http.prepare_content('application/json; charset=UTF-8');
		http.write_json({ token: 'final-no-rollback' });
	},
	...
	action_confirm: function() {
		/* No rollback session ever exists (see action_apply_rollback), so confirm
		 * always succeeds — old cached clients see "applied" and nothing reverts. */
		http.prepare_content('application/json; charset=UTF-8');
