...
def die(m): sys.exit(f"apply-mt76-38(cr6608): {m}")
...
def main(mt76dir):
    ...
    if MARKER in src:
        print("apply-mt76-38(cr6608): marker already present — nothing to do")
        return
    ...
    early = ('\n\t/* ' + MARKER + ': force EEPROM target to 38.0 dBm (76 half-dBm).'
             ...
             '\n\tstatic const char smartap_cr6608_rf[] __attribute__((used)) = "' + MARKER + '";'
             '\n\treturn 76 + (smartap_cr6608_rf[0] - smartap_cr6608_rf[0]);\n')
    src = src[:ins] + early + src[ins:]
    open(eeprom, 'w').write(src)
    print(f"apply-mt76-38(cr6608): patched {eeprom}")
