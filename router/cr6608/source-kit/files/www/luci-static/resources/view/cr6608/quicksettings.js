...
return view.extend({
	...
	FIELDS: ['mode','mesh_role','lan_ipaddr','lan_netmask','vlan_id','ssid','ssid5','smart_connect',
		...
		'wds_ssid','hide_ssid','security','wifi_key',
		'clear_previous','pppoe_user','pppoe_pass','pppoe_port','pppoe_vlan_enabled','pppoe_vlan_id','reset_lock',
	...
	applyTimeoutMs: function(payload) {
		function needsDfsWindow(channel, enabled) {
		...
		payload = payload || {};
		return needsDfsWindow(payload.channel5, payload.radio1_enabled) ? 710000 : 90000;
	},

	recoverPendingApply: function(payload) {
		var self = this, attempt = 0, maxAttempts = 20;
		function waitAndRetry() {
			return new Promise(function(resolve) { window.setTimeout(resolve, 2000); }).then(probe);
		}
		function probe() {
			attempt += 1;
			return self.requestJson('/cgi-bin/dashctl?section=apply_status', {
				method: 'GET', credentials: 'same-origin', cache: 'no-store'
			}, 5000).then(function(result) {
				var r = result.response, status = result.data || {};
				if (r.status === 401 || r.status === 403) {
					window.location.href = '/';
					return null;
				}
				if (!r.ok || status.ok !== true)
					return attempt < maxAttempts ? waitAndRetry() : null;
				if (status.pending === true && status.confirmation_ready === true &&
				    /^[0-9a-f]{32}$/.test(String(status.rollback_token || ''))) {
					return {
						ok: true,
						pending_confirmation: true,
						rollback_token: String(status.rollback_token),
						rollback_timeout_s: 900,
						rollback_remaining_s: Number(status.remaining_s),
						management_ip: String((payload && payload.lan_ipaddr) || '')
					};
				}
				if (status.busy === true || status.pending === true)
					return attempt < maxAttempts ? waitAndRetry() : null;
				return null;
			}).catch(function() {
				return attempt < maxAttempts ? waitAndRetry() : null;
	...
	handleSaveApply: function(ev, mode) {
		...
		return this.handleSave(ev).then(function() {
			...
			// A network reload can complete after disconnecting the original HTTP
			// response. Recover the authoritative Safe Apply token instead of
			// claiming failure while a rollback timer is active.
			return self.recoverPendingApply(submittedPayload).then(function(recovered) {
				if (recovered && recovered.pending_confirmation) {
					self.showReachabilityConfirmation(recovered);
					return;
				}
				releaseApplyButton();
	...
	render: function() {
		...
		o.default = '0';
		o.description = _('Explicit takeover: remove conflicting legacy/LuCI mode and VLAN settings. Leave this off to preserve settings owned outside Smart AP.');
		...
		o = s.taboption('device', form.Flag, 'pppoe_vlan_enabled', _('Use VLAN tagging on PPPoE WAN'));
		o.default = '0';
		o.depends('mode', 'pppoe');
		o.description = _('Enable only when the ISP requires 802.1Q on the WAN link. Bare WAN remains the default.');

		o = s.taboption('device', form.Value, 'pppoe_vlan_id', _('PPPoE WAN VLAN ID'));
		o.datatype = 'range(1,4094)';
		o.rmempty = false;
		o.placeholder = '35';
		o.depends({ mode: 'pppoe', pppoe_vlan_enabled: '1' });
		o.description = _('Creates an explicitly owned wan.<VID> 802.1Q device. This is independent from the AP/LAN VLAN field.');

		o = s.taboption('device', form.DummyValue, '_pppoe_wan_status', _('Current PPPoE WAN link'));
		o.depends('mode', 'pppoe');
		o.cfgvalue = function() {
			var device = String(uci.get('network', 'wan', 'device') || 'wan');
			var match = /^wan\.([0-9]{1,4})$/.exec(device);
			var managed = uci.get('network', 'cr6608_wan_vlan');
			if (device === 'wan' && !managed)
				return _('Bare WAN (untagged)');
			if (match && uci.get('network', 'cr6608_wan_vlan', 'cr6608_owner') === 'quicksettings-wan-vlan-v1' &&
			    uci.get('network', 'cr6608_wan_vlan', 'type') === '8021q' &&
			    uci.get('network', 'cr6608_wan_vlan', 'ifname') === 'wan' &&
			    String(uci.get('network', 'cr6608_wan_vlan', 'vid') || '') === match[1] &&
			    uci.get('network', 'cr6608_wan_vlan', 'name') === device)
				return _('Tagged WAN: ') + device;
			return _('Invalid or externally owned WAN mapping (apply is blocked)');
		};
		o.description = _('Live UCI ownership status. Tagged mode is accepted only when the complete managed-device record matches.');
		...
		o = s.taboption('advanced', form.DummyValue, '_nat_mode_policy', _('NAT'));
		o.cfgvalue = function() {
			return uci.get('cr6608quick', 'default', 'mode') === 'pppoe'
				? _('Enabled (owned by PPPoE mode)')
				: _('Disabled (owned by bridge/AP mode)');
		};
		o.description = _('This status follows the operating mode and is not an independent switch.');

		o = s.taboption('advanced', form.DummyValue, '_dhcp_mode_policy', _('DHCP Server'));
		o.cfgvalue = function() {
			return uci.get('cr6608quick', 'default', 'mode') === 'pppoe'
				? _('Enabled (owned by PPPoE mode)')
				: _('Disabled (owned by bridge/AP mode)');
		};
		o.description = _('Bridge, VLAN, Mesh and WDS modes use the upstream DHCP server.');

		o = s.taboption('advanced', form.Flag, 'firewall_enabled', _('جدار الحماية'));
		o.default = '1';

		o = s.taboption('advanced', form.DummyValue, '_broadband_mode_policy', _('البرودباند'));
		o.cfgvalue = function() {
			return uci.get('cr6608quick', 'default', 'mode') === 'pppoe'
				? _('Enabled on the dedicated WAN port')
				: _('Disabled; device is operating as a bridge/AP');
		};
		o.description = _('Broadband routing is owned by PPPoE mode and cannot diverge from it.');
