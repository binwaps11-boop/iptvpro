(function () {
  ...
  var UI_VERSION = "cr6608-smartap-v86-live-design-27.0.0";
  var AUTH_TIMEOUT_MS = 12000;
  var API_TIMEOUT_MS = 15000;
  // A navigation tap paints immediately, but waits for one quiet second before
  // starting a heavyweight control CGI.  Real mobile/browser scheduling can
  // pause for more than 500 ms; the wider guard prevents rapid
  // quick -> isolation -> overview gestures from leaving uncancellable CGI
  // workers queued on the single-core router.
  var CONTROL_NAV_DEBOUNCE_MS = 1000;
  var DATA_NAV_DEBOUNCE_MS = 180;
  var CONTROL_READ_RETRY_MS = 750;
  var CONTROL_READ_MAX_RETRIES = 3;
  ...
  var POLL_INTERVALS = [5, 15, 30, 60];
  var MAX_PAGE_CLIENT_MACS = 128;
  function validPollInterval(value) {
    var interval = Number(value);
    return POLL_INTERVALS.indexOf(interval) >= 0 ? interval : 5;
  }
  // Smart AP is deliberately live-only. Preferences and bounded charts live in
  // this JavaScript object for the lifetime of the open page and are discarded
  // on reload, logout, or a new authentication boundary.
  var initialPollInterval = validPollInterval(5);
  var state = {
    lang: "ar",
    theme: "dark",
    ...
    knownMacs: {},
    clientMacLru: [],
    clientInventoryOverflow: false,
    deviceNames: {},
    liveSessionSummary: null,
    liveTransitions: { events:[] },
    fetchSamples: [],
    toastTimer: 0,
    session: "",
    loginPending: false,
    ...
    maxActiveDataRequests: 0,
    controlGeneration: 0,
    controlReadController: null,
    controlReadBox: null,
    controlReadGeneration: 0,
    controlReadFailureSection: "",
    controlReadFailureCount: 0,
    controlLoadTimer: 0,
    postLock: null,
    lastFullAt: 0,
    lastFreshAt: 0,
    ...
    suspendAllHistory: false,
    lastScan: null,
    lanScan: null,
    insightCategory: "System & Health",
    sectionViewState: {},
    uiPatchGeneration: 0,
  ...
  var L = {
    ar: {
      ...
      port: "المنفذ", interface: "الواجهة", host: "الاسم", deviceName: "اسم الجهاز", scanAgain: "إعادة الفحص",
      ...
      preview: "معاينة", previewMode: "الوضع", previewManagement: "IP الإدارة (Wi-Fi + الكيبل)", previewVlan: "VLAN", previewWan: "رابط PPPoE WAN",
      ...
      passwordHint: "كلمة مرور لوحة Smart AP الخاصة بهذا الجهاز", login: "دخول", logout: "خروج",
      loginWait: "جاري التحقق من بيانات الدخول...", loginBad: "رفض الراوتر بيانات الدخول. أعد كتابة كلمة المرور يدوياً بعد حذف التعبئة المحفوظة، واستخدم root أو admin.",
      sessionExpired: "انتهت جلسة الإدارة. أدخل كلمة مرور لوحة Smart AP مجدداً.",
      loginRateLimited: "محاولات كثيرة. انتظر دقيقة ثم استخدم كلمة مرور لوحة Smart AP.",
    ...
    en: {
      ...
      port: "Port", interface: "Interface", host: "Name", deviceName: "Device name", scanAgain: "Scan again",
      ...
      preview: "Preview", previewMode: "Mode", previewManagement: "Management IP (Wi-Fi + cable)", previewVlan: "VLAN", previewWan: "PPPoE WAN link",
      ...
      passwordHint: "This device's Smart AP dashboard password", login: "Sign in", logout: "Logout",
      loginWait: "Checking credentials...", loginBad: "The router rejected these credentials. Clear any saved autofill, retype the password, and use root or admin.",
      sessionExpired: "The management session expired. Enter the Smart AP dashboard password again.",
      loginRateLimited: "Too many attempts. Wait one minute, then use the Smart AP dashboard password.",
  ...
  function patchNodeKey(node) {
    if (!node || node.nodeType !== 1) return "";
    var explicit = node.getAttribute("data-ui-key");
    if (explicit) return "key:" + explicit;
    if (node.id) return "id:" + node.id;
    var stableAttributes = [
      "data-ctl-field", "data-wizard-tab", "data-isolation-tab",
      "data-insight-category", "data-action", "data-section", "data-ctl-action"
    ];
    for (var i = 0; i < stableAttributes.length; i++) {
      var name = stableAttributes[i];
      if (node.hasAttribute(name)) return name + ":" + node.getAttribute(name);
    }
    return "";
  }
  function patchNodeShapeMatches(current, desired) {
    if (!current || !desired || current.nodeType !== desired.nodeType) return false;
    if (current.nodeType !== 1) return true;
    return current.tagName === desired.tagName;
  }
  function runtimeAttribute(name, node) {
    if (name === "data-ui-dirty" || name === "data-ui-patch-generation") return true;
    if (node && node.tagName === "CANVAS" &&
        (name === "width" || name === "height" || name === "data-chart-generation" || name === "data-chart-stable")) return true;
    return false;
  }
  function patchAttributes(current, desired) {
    var preserveValue = /^(INPUT|TEXTAREA|SELECT)$/.test(current.tagName) &&
      (current.dataset.uiDirty === "1" || document.activeElement === current);
    var value = preserveValue ? current.value : null;
    var checked = preserveValue && "checked" in current ? current.checked : null;
    var selectionStart = preserveValue && typeof current.selectionStart === "number" ? current.selectionStart : null;
    var selectionEnd = preserveValue && typeof current.selectionEnd === "number" ? current.selectionEnd : null;
    var selectionDirection = preserveValue && current.selectionDirection ? current.selectionDirection : "none";
    Array.prototype.slice.call(current.attributes).forEach(function (attribute) {
      if (runtimeAttribute(attribute.name, current)) return;
      if (!desired.hasAttribute(attribute.name)) current.removeAttribute(attribute.name);
    });
    Array.prototype.forEach.call(desired.attributes, function (attribute) {
      if (runtimeAttribute(attribute.name, current)) return;
    ...
    if (preserveValue) {
      current.value = value;
      if (checked !== null) current.checked = checked;
      if (selectionStart !== null && typeof current.setSelectionRange === "function") {
        try { current.setSelectionRange(selectionStart, selectionEnd, selectionDirection); } catch (_) {}
      }
    } else if (/^(INPUT|TEXTAREA)$/.test(current.tagName)) {
      current.value = desired.value;
      if ("checked" in current) current.checked = desired.checked;
    }
  }
  function patchDomNode(current, desired) {
    if (!patchNodeShapeMatches(current, desired)) {
      var replacement = document.importNode(desired, true);
    ...
    if (current.getAttribute("data-ui-preserve-children") === "1" && desired.getAttribute("data-ui-preserve-children") === "1")
      return current;
    patchDomChildren(current, desired);
    // A select's desired value is only meaningful after its OPTION children
    // have been reconciled. Focused/edited controls remain page-authoritative
    // until a successful mutation explicitly clears data-ui-dirty.
    if (current.tagName === "SELECT" && current.dataset.uiDirty !== "1" && document.activeElement !== current)
      current.value = desired.value;
    return current;
  }
  function patchDomChildren(currentParent, desiredParent) {
    var currentChildren = Array.prototype.slice.call(currentParent.childNodes);
    var keyed = Object.create(null), used = [];
    currentChildren.forEach(function (node, index) {
      var key = patchNodeKey(node);
      if (key && keyed[key] === undefined) keyed[key] = index;
    });
    var unkeyedCursor = 0;
    Array.prototype.slice.call(desiredParent.childNodes).forEach(function (desired, desiredIndex) {
      var desiredKey = patchNodeKey(desired), matchIndex = -1;
      if (desiredKey && keyed[desiredKey] !== undefined && !used[keyed[desiredKey]]) {
        matchIndex = keyed[desiredKey];
      } else if (!desiredKey) {
        for (var index = unkeyedCursor; index < currentChildren.length; index++) {
          if (used[index] || patchNodeKey(currentChildren[index])) continue;
          if (!patchNodeShapeMatches(currentChildren[index], desired)) continue;
          matchIndex = index;
          unkeyedCursor = index + 1;
          break;
        }
      }
      var live;
      if (matchIndex >= 0) {
        ...
        live = patchDomNode(currentChildren[matchIndex], desired);
      } else {
        live = document.importNode(desired, true);
      }
      var position = currentParent.childNodes[desiredIndex];
      if (position !== live) currentParent.insertBefore(live, position || null);
    });
    currentChildren.forEach(function (node, index) {
      if (!used[index] && node.parentNode === currentParent) currentParent.removeChild(node);
    });
  }
  function continuityKey(node) {
    if (!node || node.nodeType !== 1) return "";
    if (node.id) return "id:" + node.id;
    for (var i = 0; i < ["data-ui-key","data-ctl-field","data-insight-category","data-wizard-tab","data-isolation-tab"].length; i++) {
      var name = ["data-ui-key","data-ctl-field","data-insight-category","data-wizard-tab","data-isolation-tab"][i];
      if (node.hasAttribute(name)) return name + ":" + node.getAttribute(name);
    }
    return "";
  }
  function findContinuityNode(root, key) {
    if (!root || !key) return null;
    var nodes = [root].concat(Array.prototype.slice.call(root.querySelectorAll("*")));
    for (var i = 0; i < nodes.length; i++) if (continuityKey(nodes[i]) === key) return nodes[i];
    return null;
  }
  function capturePatchContinuity(root, options) {
    var active = document.activeElement, focus = null;
    if (active && root.contains(active)) {
      focus = { node:active, key:continuityKey(active) };
    ...
    var disclosures = [];
    Array.prototype.forEach.call(root.querySelectorAll("details,[aria-expanded]"), function (node) {
      var key = continuityKey(node);
      if (!key) return;
      disclosures.push({ key:key, open:node.tagName === "DETAILS" ? node.open : node.getAttribute("aria-expanded") === "true" });
    });
    var tabs = [];
    if (options && options.preserveTabs) {
      Array.prototype.forEach.call(root.querySelectorAll('[role="tab"][aria-selected="true"]'), function (node) {
        var key = continuityKey(node);
        if (key) tabs.push({ key:key, controls:node.getAttribute("aria-controls") || "" });
      });
    }
    var pageScroll = null, main = document.querySelector(".main");
    var activeSection = document.body.dataset.activeSection || "overview";
    var activeRoot = $(activeSection);
    if (root.isConnected && main && main.contains(root) && activeRoot &&
        (activeRoot === root || activeRoot.contains(root) || root.contains(activeRoot))) {
      pageScroll = {
        section:activeSection,
        main:main,
        mainTop:main.scrollTop,
        mainLeft:main.scrollLeft,
        windowTop:typeof window.scrollY === "number" ? window.scrollY : 0,
        windowLeft:typeof window.scrollX === "number" ? window.scrollX : 0
      };
    }
    return {
      focus:focus,
  ...
  function restorePatchContinuity(root, saved) {
    if (!saved) return;
    root.scrollTop = saved.scrollTop;
    root.scrollLeft = saved.scrollLeft;
    saved.disclosures.forEach(function (item) {
      var node = findContinuityNode(root, item.key);
      if (!node) return;
      if (node.tagName === "DETAILS") node.open = item.open;
      else node.setAttribute("aria-expanded", item.open ? "true" : "false");
    });
    (saved.tabs || []).forEach(function (item) {
      var selected = findContinuityNode(root, item.key);
      var tablist = selected && selected.closest('[role="tablist"]');
      if (!selected || !tablist || !root.contains(tablist)) return;
      Array.prototype.forEach.call(tablist.querySelectorAll('[role="tab"]'), function (tab) {
        ...
        var panel = controlled ? document.getElementById(controlled) : null;
        if (panel && root.contains(panel)) panel.hidden = !isSelected;
      });
    });
    if (saved.focus) {
      var target = saved.focus.node && saved.focus.node.isConnected ? saved.focus.node : findContinuityNode(root, saved.focus.key);
      if (target && document.activeElement !== target) {
        try { target.focus({ preventScroll:true }); } catch (_) { try { target.focus(); } catch (_) {} }
      }
      if (target && saved.focus.start !== undefined && typeof target.setSelectionRange === "function") {
        try { target.setSelectionRange(saved.focus.start, saved.focus.end, saved.focus.direction); } catch (_) {}
      }
    }
    if (saved.pageScroll) {
      var pageScroll = saved.pageScroll;
      function restorePageScroll() {
        var activeSection = document.body.dataset.activeSection || "overview";
        var activeRoot = $(pageScroll.section);
        if (!root.isConnected || activeSection !== pageScroll.section || !activeRoot ||
            !(activeRoot === root || activeRoot.contains(root) || root.contains(activeRoot))) return;
        if (pageScroll.main && pageScroll.main.isConnected)
          pageScroll.main.scrollTo({ top:pageScroll.mainTop, left:pageScroll.mainLeft, behavior:"auto" });
        window.scrollTo({ top:pageScroll.windowTop, left:pageScroll.windowLeft, behavior:"auto" });
      }
      restorePageScroll();
      if (typeof requestAnimationFrame === "function") requestAnimationFrame(restorePageScroll);
    }
  }
  function parsePatchMarkup(markup) {
    var parser = new DOMParser();
    var parsed = parser.parseFromString("<!doctype html><html><body><div id=\"smartap-patch-root\">" + String(markup || "") + "</div></body></html>", "text/html");
    return parsed.getElementById("smartap-patch-root");
  }
  function patchHtml(root, markup, options) {
    if (!root) return null;
    var desired = parsePatchMarkup(markup);
    if (!desired) return root;
    var continuity = capturePatchContinuity(root, options);
    patchDomChildren(root, desired);
    root.dataset.uiPatchGeneration = String(++state.uiPatchGeneration);
    restorePatchContinuity(root, continuity);
    return root;
  }
  ...
  function cancelDataRead() {
    if (state.dataRefreshTimer) clearTimeout(state.dataRefreshTimer);
    state.dataRefreshTimer = 0;
    state.dataRefreshReason = "";
    state.dataGeneration++;
    if (state.dataController) {
      try { state.dataController.abort(); } catch (_) {}
    }
    publishDataRequestDebug();
  }
  function publishDataRequestDebug() {
    if (typeof window === "undefined") return;
    // Operational counters only: no telemetry, credentials, or router payloads.
    window.__smartApDebug = {
      stableDom:"pass",
      stable_dom:"pass",
      activeDataRequests:state.activeDataRequests,
      maxActiveDataRequests:state.maxActiveDataRequests,
      pendingDataRefresh:!!state.dataRefreshTimer,
      dataRefreshReason:state.dataRefreshReason,
      uiPatchGeneration:state.uiPatchGeneration
    };
  }
  function countDataRequest(delta) {
    state.activeDataRequests = Math.max(0, state.activeDataRequests + delta);
    state.maxActiveDataRequests = Math.max(state.maxActiveDataRequests, state.activeDataRequests);
    publishDataRequestDebug();
  }
  function cancelControlRead() {
    if (state.controlLoadTimer) clearTimeout(state.controlLoadTimer);
    state.controlLoadTimer = 0;
    ...
    state.controlReadGeneration = 0;
  }
  function applyNavSearchFilter(value) {
    var navEl = document.querySelector(".nav");
    if (!navEl) return;
    var query = String(value || "").trim().toLowerCase();
    Array.prototype.forEach.call(navEl.querySelectorAll("button"), function (button) {
      button.style.display = (!query || button.textContent.toLowerCase().indexOf(query) > -1) ? "" : "none";
    });
  }
  function updateNavSearchCopy() {
    var search = $("navSearch");
    if (!search) return;
    search.placeholder = state.lang === "ar" ? "بحث سريع…" : "Quick search…";
    search.setAttribute("aria-label", state.lang === "ar"
      ? "بحث سريع في أقسام اللوحة"
      : "Quick search in dashboard sections");
  }
  function resetNavSearchFilter() {
    var search = $("navSearch");
    if (search) search.value = "";
    applyNavSearchFilter("");
  }
  function resetLiveSessionState() {
    // A browser tab can authenticate again without reloading. Treat that as a
    // new privacy boundary: no snapshot, client inventory, history, event, or
    // control result from the prior session may remain observable while the
    // new session is waiting for its first authoritative API response.
    cancelDataRead();
    cancelControlRead();
    clearStaleRetry();
    state.latest = null;
    state.liveDataUnavailable = false;
    state.previousTraffic = null;
    state.previousAt = 0;
    state.histories = {};
    state.liveAvailability = [];
    state.events = [];
    state.knownMacs = {};
    state.clientMacLru = [];
    state.clientInventoryOverflow = false;
    state.deviceNames = {};
    state.liveSessionSummary = null;
    state.liveTransitions = { events:[] };
    state.fetchSamples = [];
    state.pendingAction = null;
    state.adminSelection = {};
    state.controlCache = {};
    state.controlTokens = {};
    state.controlRecoveryTargets = {};
    state.controlReadFailureSection = "";
    state.controlReadFailureCount = 0;
    state.lastFullAt = 0;
    state.lastFreshAt = 0;
    state.lastLatency = 0;
    state.fullRefreshRequested = false;
    state.dataRefreshReason = "";
    state.activeDataRequests = 0;
    state.maxActiveDataRequests = 0;
    state.nextStaleRetryAt = 0;
    state._staleRefreshAttempts = 0;
    state.apiFails = 0;
    state._apiToastShown = false;
    state.suspendHistory = false;
    state.suspendAllHistory = false;
    state.lastScan = null;
    state.lanScan = null;
    state.selftest = null;
    state.sectionViewState = {};
    state.uiPatchGeneration = 0;
    window.__lastApi = null;
    publishDataRequestDebug();

    // Queued chart closures can retain the last telemetry object even after
    // their canvas has been cleared. Replacing the registry invalidates every
    // queued job; its callback observes that it is no longer authoritative.
    chartDrawJobs = Object.create(null);

    if ($("loginPass")) $("loginPass").value = "";
    resetNavSearchFilter();
    document.querySelectorAll(".smartap-ephemeral-overlay").forEach(function (node) {
    ...
    if ($("kpiBar")) clearNode($("kpiBar"));
    if ($("snapshotWarning")) {
      $("snapshotWarning").hidden = true;
      $("snapshotWarning").textContent = "";
    }
    if ($("connectionState")) $("connectionState").textContent = tr("loading");
    if ($("connectionChip")) $("connectionChip").classList.remove("ok", "warn", "bad");
    if ($("statusPulse")) $("statusPulse").classList.remove("warn", "bad");
    if ($("sideTitle")) $("sideTitle").textContent = "Smart AP";
    if ($("sideStatus")) $("sideStatus").textContent = tr("loading");
    document.body.dataset.activeSection = "overview";
    document.body.dataset.liveState = "loading";
    ["overview","network","devices","wifi","insights","system","quick","isolation","actions"].forEach(function (id) {
      var section = $(id);
      if (!section) return;
      clearNode(section);
      section.hidden = id !== "overview";
      delete section.dataset.uiVersion;
      delete section.dataset.loaded;
      delete section.dataset.liveState;
    });
  }
  ...
  function scheduleStaleRetry(delay) {
    ...
    state.staleRetryTimer = setTimeout(function () {
      state.staleRetryTimer = 0;
      if (state.session && !state.navigationPending) scheduleDataRefresh(0, true, "stale-retry");
    }, Math.max(250, Number(delay) || 0));
  }
  function scheduleDataRefresh(delay, forceFull, reason) {
    if (!state.session || state.navigationPending) return false;
    if (state.dataRefreshTimer) clearTimeout(state.dataRefreshTimer);
    state.dataRefreshReason = String(reason || "scheduled");
    state.dataRefreshTimer = setTimeout(function () {
      state.dataRefreshTimer = 0;
      var scheduledReason = state.dataRefreshReason;
      state.dataRefreshReason = "";
      publishDataRequestDebug();
      if (state.session && !state.navigationPending) loadData(forceFull === true, scheduledReason);
  ...
  function queueFullRefresh(delay) {
    state.lastFullAt = 0;
    state.nextStaleRetryAt = 0;
    return scheduleDataRefresh(delay, true, "full-refresh");
  }
  ...
  function controlStatusNode(box, key, className) {
    if (!box) return null;
    var selector = '[data-ui-key="' + key + '"]';
    var node = box.querySelector ? box.querySelector(selector) : null;
    if (!node) {
      node = document.createElement("div");
      node.dataset.uiKey = key;
      if (box.firstChild) box.insertBefore(node, box.firstChild); else box.appendChild(node);
    }
    node.className = className || "ctl-status";
    return node;
  }
  function paintControlNotice(box, message, warning) {
    var node = controlStatusNode(box, "control-notice", "ctl-status" + (warning ? " warn" : ""));
    if (node) {
  ...
  function paintControlRecovery(box, message, context, warning) {
    if (!box) return;
    var node = paintControlNotice(box, message, warning);
    var targetUrl = context && context.targetUrl || "";
    if (!targetUrl || !node) return;
  ...
  function startControlProgress(box, actionName) {
    ...
    function paint() {
      ...
      var progress = controlStatusNode(box, "control-progress", "ctl-status");
      box.setAttribute("aria-busy", "true");
      if (progress) progress.textContent = (state.lang === "ar"
    ...
    return function () {
      ...
      if (box) {
        ...
        var progress = box.querySelector ? box.querySelector('[data-ui-key="control-progress"]') : null;
  ...
  function presentPendingApply(section, box, data) {
    ...
    patchHtml(box, '<div class="ctl-card warn"><span>Safe Apply</span><b>' + esc(data.summary || "Confirmation pending") + '</b><small class="latin">' + esc(remaining) + '</small></div>' +
  ...
  async function recoverControlAction(section, actionName, box, recoveryContext) {
    ...
    function releaseIfInactive() {
      if (activeControlSection() === section && box.isConnected !== false) return false;
    ...
    while (Date.now() < deadline) {
      // Recovery owns the loaded marker only while this exact panel is active.
      // If navigation replaces/hides it, leave the panel reloadable so a later
      // visit fetches the authoritative Safe Apply state from the router.
      if (releaseIfInactive()) return true;
      var reconnectMessage = state.lang === "ar"
      ...
      paintControlRecovery(box, reconnectMessage, recoveryContext, false);
      try {
        var status = await readApplyStatus();
        // Navigation can happen while the authoritative probe is in flight.
        // Never paint its completion into an old or disconnected panel.
        if (releaseIfInactive()) return true;
        ...
        if (releaseIfInactive()) return true;
        if (probeError && probeError.code === "SMARTAP_AUTH") {
          delete box.dataset.loaded;
          return true;
        }
      }
    ...
    if (releaseIfInactive()) return true;
    var timeoutMessage = state.lang === "ar"
    ...
    paintControlRecovery(box, timeoutMessage, recoveryContext, true);
    return true;
  }
  async function waitForRouterReachable(name, button) {
    ...
    function restoreButton() {
      ...
      if (oldHtml != null) patchHtml(button, oldHtml);
    }
    if (button) { button.disabled = true; button.setAttribute("aria-busy", "true"); }
    while (Date.now() < deadline) {
      if (button) button.textContent = state.lang === "ar" ? "جارٍ إعادة الاتصال…" : "Reconnecting…";
      try {
        var res = await fetchWithTimeout(authUrl(API + "?lite=1&live=1"), { credentials:"same-origin", cache:"no-store", headers:authHeaders() }, RECOVERY_PROBE_TIMEOUT_MS);
  ...
  function spark(values, color) {
    ...
    if (values.length < 2) return '<svg class="spark" viewBox="0 0 120 24" aria-hidden="true" focusable="false"><path d="M4 18H116" stroke="rgba(148,163,184,.35)" stroke-width="2" stroke-linecap="round"/></svg>';
    ...
    return '<svg class="spark" viewBox="0 0 120 24" aria-hidden="true" focusable="false"><path d="' + d + '" fill="none" stroke="' + color + '" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  }
  function touchPageClient(mac) {
    var key = String(mac || "").replace(/^\s+|\s+$/g, "").toLowerCase();
    if (!/^[0-9a-f]{2}(:[0-9a-f]{2}){5}$/.test(key)) return "";
    var order = state.clientMacLru || (state.clientMacLru = []);
    var previous = order.indexOf(key);
    if (previous >= 0) order.splice(previous, 1);
    order.push(key);
    while (order.length > MAX_PAGE_CLIENT_MACS) {
      var oldest = order.shift();
      delete state.knownMacs[oldest];
      delete state.deviceNames[oldest];
      delete state.histories["sig_" + oldest];
      delete state.histories["rate_" + oldest];
    }
    return key;
  }
  function pushHistory(key, value, max, allowWhenSuspended) {
    if (state.suspendAllHistory) return;
  ...
  function gauge(title, value, unit, sub, percent, color, histKey) {
    ...
    var summary = String(title || "") + ": " + String(value == null ? "" : value) + (unit ? " " + unit : "");
    return '<div class="gauge-card" style="box-shadow:0 0 34px ' + color + '18">' +
      '<div class="gauge" aria-hidden="true"><svg viewBox="0 0 132 132" focusable="false">' +
      ...
      '</svg></div><span class="sr-only gauge-summary">' + esc(summary) + '</span><div class="muted" style="text-align:center">' + esc(sub || "") + '</div>' +
  ...
  function tableCaption(label) { return '<caption class="sr-only">' + esc(label) + '</caption>'; }
  function tableHeaderCells(labels) {
    return labels.map(function (label) { return '<th scope="col">' + esc(label) + '</th>'; }).join("");
  }
  // title/chip are always plain text; body is the only trusted-HTML argument.
  function card(title, body, chip, iconName, uiKey) {
    return '<article class="card"' + (uiKey ? ' data-ui-key="card:' + esc(uiKey) + '"' : '') + '><div class="card-head"><div class="title">' + icon(iconName || "bolt") + '<span>' + esc(title) + '</span></div>' + (chip ? '<span class="chip">' + esc(chip) + '</span>' : "") + '</div>' + body + '</article>';
  }
  function sectionHead(title, desc, chip) {
    return '<div class="section-head" data-ui-key="section-head"><div><h3>' + esc(title) + '</h3><p>' + esc(desc) + '</p></div>' + (chip ? '<span class="chip">' + esc(chip) + '</span>' : "") + '</div>';
  }
  ...
  // Canvas nodes remain mounted across live refreshes. Each drawing job owns one
  // generation and must paint that current connected node at a stable size on
  // two consecutive frames. Older jobs can never paint a newer generation.
  ...
  // Render helpers register chart jobs before the keyed patch is applied. Paint
  // the mounted canvas synchronously once, retaining the queued second pass for
  // stable-size and background-tab recovery.
  ...
  function redrawMountedCharts(root) {
    if (!root || !root.querySelectorAll) return;
    Array.prototype.forEach.call(root.querySelectorAll("canvas[id]"), function (canvas) {
      var job = chartDrawJobs[canvas.id];
      if (!job || typeof job.paint !== "function") return;
      try { job.paint(); } catch (_) {}
    });
  }
  ...
  function dataUsage(data) {
    ...
    // counters: interface reloads and power loss reset them, and page-memory
  ...
  // Custom names are page-session annotations only. They intentionally vanish
  // when this page is reloaded so client identity is never persisted by Smart AP.
  function devNamesMap() { return state.deviceNames; }
  function deviceName(mac) { if (!mac) return ""; return state.deviceNames[String(mac).toLowerCase()] || ""; }
  function setDeviceName(mac, name) {
    if (!mac) return; var m = state.deviceNames; var k = String(mac).toLowerCase();
    if (name) m[k] = String(name).slice(0, 32); else delete m[k];
  }
  function mergeDevices(data) {
    var map = {};
    (data.devices || []).forEach(function (d) {
      var k = String(d.mac || d.ip || [d.type || "Ethernet", d.iface || "", d.host || "unknown"].join("|")).toLowerCase();
      map[k] = { ip:d.ip, mac:d.mac, iface:d.iface, type:d.type || "Ethernet", host:d.host, _uiKey:k };
    });
    (data.wifi || []).forEach(function (w) { (w.stations || []).forEach(function (s) {
      var k = String(s.mac || s.ip || ["WiFi", w.iface || w.radio || w.band || "radio", s.host || "unknown"].join("|")).toLowerCase();
      var t = stationTraffic(s);
      map[k] = Object.assign(map[k] || {}, { mac:s.mac, ip:s.ip || (map[k]||{}).ip, iface:w.iface, type:"WiFi", signal:s.signal_dbm, rate:s.tx_rate, txRate:s.tx_rate, rxRate:s.rx_rate, tx_rate:s.tx_rate, rx_rate:s.rx_rate, tx_rate_detail:s.tx_rate_detail, rx_rate_detail:s.rx_rate_detail, tx_retries:s.tx_retries, tx_failed:s.tx_failed, inactive_ms:s.inactive_ms, down:t.down, up:t.up, downBytes:t.downBytes, upBytes:t.upBytes, hasTraffic:t.hasCounters, _uiKey:k });
    }); });
  ...
  function updateAvailability(ok) {
    state.liveAvailability.push({ t:Date.now(), ok:!!ok });
    state.liveAvailability = state.liveAvailability.slice(-60);
  }
  function availabilityHtml() {
    if (!state.liveAvailability.length) return '<div class="empty">' + tr("loading") + '</div>';
    var cells = state.liveAvailability.map(function (sample) {
      var color = sample.ok ? "var(--excellent)" : "var(--weak)";
      var status = sample.ok ? (state.lang === "ar" ? "عينة حية متاحة" : "live sample available") : (state.lang === "ar" ? "عينة حية غير متاحة" : "live sample unavailable");
      return '<i title="' + esc(new Date(sample.t).toLocaleTimeString() + " · " + status) + '" style="border-radius:4px;background:' + color + '"></i>';
    });
    return '<div style="display:grid;grid-template-columns:repeat(' + cells.length + ',minmax(2px,1fr));gap:2px;height:34px">' + cells.join("") + '</div>' +
      '<small class="muted">' + esc(state.lang === "ar" ? "آخر 60 عينة حية في هذه الصفحة فقط؛ تُمسح عند إعادة التحميل أو الخروج." : "Last 60 live samples in this open page only; cleared on reload or logout.") + '</small>';
  }
  function hideToast() {
    var el = $("toast");
  ...
  function setLoginText() {
    if ($("loginMark")) patchHtml($("loginMark"), icon("wifi"));
  ...
  function normalizeMobilePassword(value) {
    // Mobile RTL clipboards may wrap an otherwise ASCII credential in newline,
    // zero-width, or bidi-control characters. They are display controls, not
    // intentional password glyphs in the per-device provisioning alphabet.
    return String(value || "")
      .replace(/[\r\n\u200b-\u200f\u202a-\u202e\u2060\u2066-\u2069]/g, "")
      .replace(/^\s+|\s+$/g, "");
  }
  function clearLoginCredentialInput() {
    var pass = $("loginPass");
    if (!pass) return;
    pass.value = "";
    pass.defaultValue = "";
  ...
  async function revokeServerSessions() {
    ...
    if (!res.ok || !data.ok) {
      var revokeError = new Error(data.error || "session revocation failed");
      // dashlogout clears every browser cookie even when server-side rpcd
      // destruction returns 503.  Mark an HTTP response distinctly from a
      // transport timeout so logout can converge locally without resurrecting
      // polling against cookies the server already expired.
      revokeError.serverResponded = true;
      throw revokeError;
    }
    return true;
  }
  ...
  async function login(username, password) {
    ...
    try {
      ...
      var pass = normalizeMobilePassword(password);
      ...
      if (e && e.status === 401 && $("loginPass")) $("loginPass").value = "";
  ...
  async function validateSession() {
    ...
    try {
      ...
      var res = await fetchWithTimeout(authUrl(API + "?lite=1&probe=1&live=1"), { credentials:"same-origin", cache:"no-store", headers:authHeaders() }, 8000);
  ...
  async function logout() {
    // Logout is a navigation/privacy boundary. Block the in-flight reader's
    // finally block from admitting a trailing full refresh while the server
    // session is being revoked; otherwise that new read is immediately aborted
    // by showLogin(), producing a redundant request and wasted collector work.
    ...
    try {
      ...
      if (e && e.serverResponded === true) {
        state.session = "";
        if ($("loginPass")) $("loginPass").value = "";
        showLogin(tr("loggedOut"), false);
        toast(e.message || tr("loginBad"));
      } else {
        // No response means the request may not have reached the router.  Keep
        // the local session and allow the operator to retry safely.
  ...
  function renderChrome() {
    ...
    $("themeBtn").textContent = state.themePreference === "auto" ? (state.lang === "ar" ? "تلقائي" : "Auto") : (state.theme === "dark" ? tr("dark") : tr("light"));
    ...
    // Keep the navigation button nodes stable across live-data, language, and
    // theme repaints. Replacing a pressed mobile button between pointer-down
    // and click can discard the user's navigation event entirely.
    var navElement = $("nav");
    var navButtons = navElement ? navElement.querySelectorAll("button[data-section]") : [];
    var navShapeValid = navButtons.length === nav.length;
    if (navShapeValid) {
      for (var navIndex = 0; navIndex < nav.length; navIndex++) {
        if (navButtons[navIndex].dataset.section !== nav[navIndex][0]) {
          navShapeValid = false;
          break;
        }
      }
    }
    if (navElement && !navShapeValid) {
      patchHtml(navElement, nav.map(function (n) {
        return '<button data-section="' + n[0] + '">' + icon(n[2]) + '<span></span></button>';
      }).join(""));
      navButtons = navElement.querySelectorAll("button[data-section]");
    }
    Array.prototype.forEach.call(navButtons, function (b, index) {
      var active = b.dataset.section === activeSection;
      var label = b.querySelector("span");
      if (label) label.textContent = tr(nav[index][1]);
  ...
  function rememberSectionView(id) {
    if (!id || !$(id)) return;
    var main = document.querySelector(".main");
    state.sectionViewState[id] = {
  ...
  function restoreSectionView(id) {
    var saved = state.sectionViewState[id] || { mainTop:0, mainLeft:0, windowTop:0, windowLeft:0 };
    function restore() {
      if ((document.body.dataset.activeSection || "overview") !== id) return;
      var main = document.querySelector(".main");
      if (main) main.scrollTo({ top:saved.mainTop, left:saved.mainLeft, behavior:"auto" });
      window.scrollTo({ top:saved.windowTop, left:saved.windowLeft, behavior:"auto" });
    }
    restore();
    if (typeof requestAnimationFrame === "function") requestAnimationFrame(restore);
  }
  function showSection(id) {
    var previousSection = document.body.dataset.activeSection || "overview";
    rememberSectionView(previousSection);
    cancelControlRead();
    var requestedControl = id === "quick" ? "wizard" : id === "isolation" ? "isolation" : "";
    if (requestedControl && state.controlReadFailureSection === requestedControl &&
        state.controlReadFailureCount >= CONTROL_READ_MAX_RETRIES)
      resetControlReadRetry(requestedControl);
    ...
    else if (state.liveDataUnavailable) {
      paintLiveUnavailableSection(id);
    }
    restoreSectionView(id);
    scheduleActiveControlLoad();
    if (state.latest && /^(network|devices|wifi|insights)$/.test(id) && Date.now() - state.lastFullAt > 5000) {
      scheduleDataRefresh(DATA_NAV_DEBOUNCE_MS, true, "navigation:" + id);
    }
  }
  ...
  function renderKpis(data, rates, liveCounters) {
    ...
    patchHtml($("kpiBar"), list.map(function (x, i) {
      var h = i === 3 ? state.liveAvailability.map(function (a) { return a.ok ? 1 : 0; }) : (i === 0 ? state.histories.kpiRx : state.histories.kpiTx);
      return '<div class="kpi" data-ui-key="kpi:' + esc(x[2]) + '"><span>' + esc(x[0]) + '</span><b>' + esc(x[1]) + '</b>' + spark(h || [], x[3] ? "#06B6D4" : "#EF4444") + '</div>';
    }).join(""));
  }
  ...
  function throughputRow(label, download, upload, up, chip, uiKey) {
    var tot = (finite(download) ? download : 0) + (finite(upload) ? upload : 0);
    var downPct = tot > 0 ? (finite(download) ? download : 0) / tot * 100 : 0;
    return '<div data-ui-key="throughput:' + esc(uiKey || label) + '" style="margin:7px 0"><div style="display:flex;justify-content:space-between;font-size:12px"><b class="latin">' + esc(label) +
  ...
  function renderPortThroughput(data) {
    ...
    var wifiRows = Object.keys(wifiAgg).map(function (b) { return throughputRow("WiFi " + b, wifiAgg[b].tx, wifiAgg[b].rx, wifiAgg[b].up, b, "wifi:" + b); }).join("");
    ...
    var portRows = ports.map(function (i) { return throughputRow(i.name, num(i.tx_bps), num(i.rx_bps), i.connected, i.connected ? (i.speed_mbps ? i.speed_mbps + "M" : "up") : "down", "port:" + String(i.name || "unknown").toLowerCase()); }).join("");
  ...
  function renderSecPosture(data) {
    ...
    var rows = w.map(function (x) {
      ...
      if (s.key === "open") open++;
      return '<div class="kv" data-ui-key="security:' + esc(String(x.iface || x.ssid || x.band || "radio").toLowerCase()) + '"><div><span class="latin">' + esc(x.ssid || x.iface) + '</span>' +
    ...
    return card(tr("secPosture"), rows, chip, "shield", "security-posture");
  }
  function renderDevices(data) {
    ...
    var scanCard = card(tr("lanNeighbors"),
      ...
      '<div id="lanScanResult">' + (state.lanScan ? renderLanScan(state.lanScan) : "") + '</div>', "LLDP / CDP", "net", "lan-scan");
    var live = rows.length ? sectionHead(tr("devices"), "IP / MAC / " + tr("vendor") + " / RSSI", rows.length + "") +
      '<div class="table-wrap"><table>' + tableCaption(tr("devices")) + '<thead><tr>' + tableHeaderCells([tr("type"), "IP", "MAC", tr("vendor"), tr("link"), "RSSI", tr("clientTraffic"), tr("action")]) + '</tr></thead><tbody>' +
      rows.map(function (d) {
        ...
        return '<tr data-ui-key="device:' + esc(d._uiKey || d.mac || d.ip || d.iface || "unknown") + '"><td>' + icon(d.type === "WiFi" ? "wifi" : "device") + " " + esc(d.type || "") + '</td><td class="latin">' + ipHost + '</td><td class="latin">' + esc((d.mac || tr("unavailable")).toUpperCase()) + '</td><td>' + esc(vn) + '</td><td>' + linkCell + '</td><td class="latin">' + (num(d.signal) !== null ? d.signal + ' dBm ' + proximity(d.signal) : tr('unavailable')) + '</td><td>' + traf + '</td><td>' + acts + '</td></tr>';
      }).join("") +
      '</tbody></table></div>' :
  ...
  function isRealMac(mac) {
    ...
    if (m === "00:00:00:00:00:00" || m === "ff:ff:ff:ff:ff:ff") return false;
  ...
  function renderLanScan(d) {
    ...
    var devices = (d.devices || []).filter(function (device) {
      return device && isRealMac(device.mac);
    });
    var lldp = (d.lldp || []);
    var sections = [];
    if (devices.length) {
      sections.push('<h4 style="margin:10px 0 6px">' + tr("lanNeighbors") + ' (' + devices.length + ')</h4>' +
        '<div class="table-wrap"><table>' + tableCaption(tr("lanNeighbors")) + '<thead><tr>' + tableHeaderCells([tr("deviceName"), "IP", "MAC", tr("port"), tr("interface")]) + '</tr></thead><tbody>' +
        devices.map(function (device) {
          return '<tr data-ui-key="lan:' + esc(String(device.mac || device.ip || [device.port, device.iface, device.host].join("|")).toLowerCase()) + '"><td>' + esc(device.host || "—") + '</td><td class="latin">' + esc(device.ip || "—") + '</td><td class="latin">' + esc(String(device.mac || "—").toUpperCase()) + '</td><td class="latin">' + esc(device.port || "—") + '</td><td class="latin">' + esc(device.iface || "—") + '</td></tr>';
        }).join("") + '</tbody></table></div>');
    }
    if (lldp.length) sections.push('<h4 style="margin:10px 0 6px">' + tr("lldpNeighbors") + ' (' + lldp.length + ')</h4>' +
      '<div class="table-wrap"><table>' + tableCaption(tr("lldpNeighbors")) + '<thead><tr>' + tableHeaderCells([tr("deviceName"), tr("platform"), "IP", tr("localPort"), tr("remotePort")]) + '</tr></thead><tbody>' +
      lldp.map(function (n) {
        return '<tr data-ui-key="lldp:' + esc([n.local_port || "", n.remote_port || "", n.name || n.ip || "unknown"].join("|")) + '"><td>' + esc(n.name || "—") + '</td><td>' + esc(n.platform || "—") + '</td><td class="latin">' + esc(n.ip || "—") + '</td><td class="latin">' + esc(n.local_port || "—") + '</td><td class="latin">' + esc(n.remote_port || "—") + '</td></tr>';
      }).join("") + '</tbody></table></div>');
    if (!sections.length) return '<div class="empty">' + esc(tr("noLanDevices")) + '</div>';
  ...
  async function scanLan() {
    ...
    patchHtml(box, '<div class="ctl-status">' + esc(tr("scanning")) + '</div>');
    try {
      ...
      patchHtml(box, renderLanScan(d));
      bindDynamic(box);
    } catch (e) {
      patchHtml(box, '<div class="ctl-status">' + esc("scan: " + e.message) + '</div>');
  ...
  function renderWifi(data) {
    ...
    var body = w.length ? '<div class="grid two">' + w.map(function (x) {
      var radioKey = String(x.iface || x.radio || x.band || "radio").toLowerCase();
      ...
      var sta = (x.stations || []).map(function (s) {
        var stationKey = radioKey + ":" + String(s.mac || s.ip || "unknown").toLowerCase();
        var ss = num(s.signal_dbm), noise = num(s.noise_dbm), snr = num(s.snr), qq = quality("rssi", ss);
        var historyMac = touchPageClient(s.mac);
        ...
        // Per-client signal trend is bounded to this open page only.
        if (historyMac && finite(ss)) pushHistory("sig_" + historyMac, ss, 40);
        var trendStr = (historyMac && (state.histories["sig_" + historyMac] || []).length > 2) ? spark(state.histories["sig_" + historyMac], qq.color) : "";
        ...
        if (historyMac && stRate.hasRate) pushHistory("rate_" + historyMac, (stRate.down || 0) + (stRate.up || 0), 40);
        var rateTrend = (historyMac && (state.histories["rate_" + historyMac] || []).length > 2) ? spark(state.histories["rate_" + historyMac], "var(--accent)") : "";
        ...
        return '<div class="kv" data-ui-key="station:' + esc(stationKey) + '"><div><span class="latin">' + esc(s.ip || s.mac || tr("unavailable")) + steer + '</span><b class="latin">' + (finite(ss) ? ss + " dBm " : "") + proximity(ss) + '</b></div>' + bar(finite(ss) ? signalPct("rssi", ss) : 0, 100, qq.color) +
    ...
    var chanCard = w.length ? card(state.lang === "ar" ? "إشغال القنوات" : "Channel occupancy",
      '<canvas id="channelCanvas" style="width:100%;height:150px"></canvas>', "2.4G / 5G", "signal", "channel-occupancy") : "";
    var radarCard = totalSta ? card(tr("clientRadar"),
      '<canvas id="constellationCanvas" style="width:100%;height:230px"></canvas><p class="muted" style="text-align:center">' + esc(tr("constellation")) + '</p>', totalSta + "", "device", "client-radar") : "";
    ...
    var scanCard = card(tr("bestChannel") + " · " + tr("neighbors"),
      ...
      '<div id="wifiScanResult">' + (state.lastScan ? renderScanResult(state.lastScan) : "") + '</div>', "iw scan", "signal", "wifi-scan");
  ...
  async function scanWifi() {
    ...
    try {
      ...
      patchHtml(box, renderScanResult(d));
      bindDynamic(box);
    } catch (e) {
      patchHtml(box, '<div class="ctl-status">' + esc("scan: " + e.message) + '</div>');
    } finally { btn.disabled = false; btn.textContent = old; }
  ...
  function renderScanResult(d) {
    ...
    var list = neigh.length ? '<div class="table-wrap"><table>' + tableCaption(tr("neighbors")) + '<thead><tr>' + tableHeaderCells(["SSID", tr("channel"), "Signal", "BSSID"]) + '</tr></thead><tbody>' +
      neigh.slice(0, 30).map(function (x) {
        var sg = num(x.signal);
        var neighborKey = String(x.bssid || [x.ssid || "", x.ch || "?"].join("|")).toLowerCase();
        return '<tr data-ui-key="neighbor:' + esc(neighborKey) + '"' + (x.rogue ? ' style="background:rgba(239,68,68,.12)"' : '') + '><td>' + esc(x.ssid || "—") + (x.rogue ? ' ⚠' : '') + '</td><td class="latin">' + esc(x.ch || "?") + '</td><td class="latin">' + (finite(sg) ? sg + " dBm " + proximity(sg) : "?") + '</td><td class="latin">' + esc((x.bssid || "").toUpperCase()) + '</td></tr>';
  ...
  function renderSystem(data) {
    ...
    return sectionHead(tr("systemTitle"), "loadavg / free / overlay / thermal", "thresholds") + card(state.lang === "ar" ? "لقطة الموارد" : "Resource snapshot", body, "live", "cpu") + trendCard + selftestCard + card(state.lang === "ar" ? "عينات التوفر الحية" : "Live availability samples", availabilityHtml(), state.lang === "ar" ? "ذاكرة الصفحة فقط" : "page memory only", "bolt");
  }
  function renderLinkDiagnostics(data) {
    var diag = data && data.link_diagnostics;
    var ports = diag && Array.isArray(diag.ports) ? diag.ports : [];
    if (!ports.length) return "";
    var ar = state.lang === "ar";
    var slow = ports.filter(function (port) {
      return port && port.connected === true && Number(port.speed_mbps) === 100 && Number(port.capability_mbps) === 1000;
    });
    var warning = slow.map(function (port) {
      var name = '<b class="latin">' + esc(port.name || "port") + '</b>';
      return '<div class="ctl-note traffic-fault">' + (ar
        ? '\u0627\u0644\u0645\u0646\u0641\u0630 ' + name + ' \u0645\u0631\u062a\u0628\u0637 \u0628\u0633\u0631\u0639\u0629 100 \u0645\u064a\u062c\u0627\u0628\u062a \u0631\u063a\u0645 \u0623\u0646\u0647 \u064a\u062f\u0639\u0645 1000 \u0645\u064a\u062c\u0627\u0628\u062a. \u0627\u0633\u062a\u062e\u062f\u0645 \u0643\u0627\u0628\u0644 Cat5e/Cat6 \u0628\u062b\u0645\u0627\u0646\u064a\u0629 \u0623\u0633\u0644\u0627\u0643\u060c \u0623\u0639\u062f \u062a\u062b\u0628\u064a\u062a \u0627\u0644\u0637\u0631\u0641\u064a\u0646\u060c \u0623\u0648 \u062c\u0631\u0651\u0628 \u0645\u0646\u0641\u0630\u0627\u064b \u0622\u062e\u0631 \u0641\u064a \u0627\u0644\u0631\u0627\u0648\u062a\u0631/\u0627\u0644\u0633\u0648\u064a\u062a\u0634.'
        : name + ' is linked at 100 Mbps although this CR6608 DSA port supports 1000 Mbps. Use an 8-wire Cat5e/Cat6 cable, reseat both ends, or try another router/switch port.') + '</div>';
    }).join("");
    var labels = ar
      ? ["\u0627\u0644\u0645\u0646\u0641\u0630", "\u0627\u0644\u0631\u0627\u0628\u0637", "\u0627\u0644\u0633\u0631\u0639\u0629 / \u0627\u0644\u0642\u062f\u0631\u0629", "Link Down", "\u0627\u0644\u0645\u0635\u062f\u0631"]
      : ["Port", "Link", "Speed / capability", "Link Down", "Evidence"];
    var rows = ports.map(function (port) {
      var connected = port.connected === true;
      var events = Number(port.link_down_events);
      return '<tr data-ui-key="link:' + esc(String(port.name || "unknown").toLowerCase()) + '"><td class="latin">' + esc(port.name || "") + '</td><td><span class="chip ' + (connected ? "ok" : "bad") + '">' +
        (connected ? (ar ? "\u064a\u0639\u0645\u0644" : "up") : (ar ? "\u0645\u062a\u0648\u0642\u0641" : "down")) +
        '</span></td><td class="latin">' + esc(port.speed_mbps || 0) + ' / ' + esc(port.capability_mbps || 1000) + ' Mbps</td><td class="latin ' +
        (events > 0 ? "traffic-fault" : "traffic-clean") + '">' + (Number.isFinite(events) && events >= 0 ? events : "\u2014") +
        '</td><td class="latin">' + esc(port.link_down_source || "unavailable") + '</td></tr>';
    }).join("");
    var scope = String(diag.count_scope || "unavailable");
    var evidenceNote = diag.complete === true
      ? (ar ? "\u0627\u0644\u0639\u062f\u0651 \u0645\u0646 \u0627\u0644\u0646\u0648\u0627\u0629 \u0645\u0646\u0630 \u062a\u0633\u062c\u064a\u0644 \u0648\u0627\u062c\u0647\u0629 \u0627\u0644\u0634\u0628\u0643\u0629 \u0641\u064a \u0627\u0644\u0625\u0642\u0644\u0627\u0639 \u0627\u0644\u062d\u0627\u0644\u064a\u061b \u0625\u0639\u0627\u062f\u0629 \u062a\u062d\u0645\u064a\u0644 \u0627\u0644\u062f\u0631\u0627\u064a\u0641\u0631 \u062a\u0639\u064a\u062f \u0627\u0644\u0639\u062f\u0651." : "Kernel count since netdevice registration in the current boot; reloading the driver resets it.")
      : (ar ? "\u0627\u0644\u0639\u062f\u0651 \u0627\u0644\u0627\u062d\u062a\u064a\u0627\u0637\u064a \u0645\u0646 \u0627\u0644\u062c\u0632\u0621 \u0627\u0644\u0645\u062a\u0627\u062d \u062d\u0627\u0644\u064a\u0627\u064b \u0641\u064a \u0633\u062c\u0644 \u0647\u0630\u0627 \u0627\u0644\u0625\u0642\u0644\u0627\u0639\u061b \u0642\u062f \u062a\u0643\u0648\u0646 \u0627\u0644\u0623\u062d\u062f\u0627\u062b \u0627\u0644\u0642\u062f\u064a\u0645\u0629 \u062d\u0630\u0641\u062a \u0645\u0646 \u0627\u0644\u0633\u062c\u0644." : "Fallback count covers only the currently available log buffer from this boot; older events may have rolled out.");
    var body = warning + '<p class="muted traffic-note">' + esc(evidenceNote) + ' <span class="latin">[' + esc(scope) + ']</span></p>' +
      '<div class="table-wrap"><table>' + tableCaption(ar ? "\u062a\u0634\u062e\u064a\u0635 \u0631\u0648\u0627\u0628\u0637 DSA" : "DSA link diagnostics") + '<thead><tr>' + tableHeaderCells(labels) +
      '</tr></thead><tbody>' + rows + '</tbody></table></div>';
    var total = Number(diag.link_down_events_total);
    return card(ar ? "\u062a\u0634\u062e\u064a\u0635 \u0631\u0648\u0627\u0628\u0637 DSA" : "DSA link diagnostics", body,
      (Number.isFinite(total) && total >= 0 ? total : 0) + " Link Down", "net");
  }
  function renderNetwork(data, rates) {
    ...
    var table = '<p class="muted traffic-note">' + (ar ? "RX/TX هنا اتجاه عداد الواجهة الخام، وليس تنزيل/رفع العميل." : "RX/TX below are raw interface-counter directions, not client download/upload.") + '</p><div class="table-wrap traffic-table"><table>' +
      tableCaption(ar ? "\u062d\u0627\u0644\u0629 \u0627\u0644\u0648\u0627\u062c\u0647\u0627\u062a \u0648\u0627\u0644\u0645\u0646\u0627\u0641\u0630" : "Interfaces and ports") + '<thead><tr>' + tableHeaderCells(labels) + '</tr></thead><tbody>' +
    ...
    return sectionHead(tr("networkTitle"), ar ? "نقل لحظي، إجماليات، أخطاء، وسرعة جلب الواجهة" : "Live throughput, totals, errors, and dashboard response", data.backhaul && data.backhaul.online ? tr("online") : tr("lanOnly")) +
      '<div class="traffic-layout"><div class="traffic-primary">' + renderTraffic(data, rates, "network") + '</div><div class="traffic-latency">' + ping + '</div></div>' + renderLinkDiagnostics(data) + interfaceCard;
  }
  ...
  function renderControlData(section, data) {
    ...
    if (data.actions && data.actions.length) {
      html += '<div class="branch-actions">' + data.actions.filter(function (a) { return !a.url; }).map(function (a) {
        ...
        return '<button class="' + cls + '" data-ctl-section="' + esc(section) + '" data-ctl-action="' + esc(a.id) + '" data-ctl-input="' + esc(a.input || "") + '" data-ctl-confirm="' + (a.confirm ? "1" : "") + '">' + esc(controlActionLabel(a)) + '</button>';
      }).join("") + '</div>';
    }
  ...
  function controlActionLabel(action) {
    if (!action) return "";
    if (state.lang !== "en") return action.label || action.id || "";
    var labels = {
      apply_royal:"Save & Apply",
      save_identity:"Save customer identity",
      reset_royal:"Reset form",
      apply_isolation:"Save & Apply",
      run_selftest:"Run self-test",
      raw_uci_set:"Set option",
      raw_uci_delete:"Delete option",
      raw_uci_add_section:"Add section",
      raw_uci_delete_section:"Delete section",
      raw_uci_commit_reload:"Commit and reload"
    };
    return labels[action.id] || action.label || action.id || "";
  }
  ...
  var CONTROL_EN = {
    sec_enabled:["General protection","Master switch for the protection policies below."],
    flood_guard:["Request flood protection","Limits abusive management requests while preserving normal access."],
    rogue_dhcp_guard:["Rogue DHCP protection","Prevents an unauthorized device from distributing incorrect addresses."],
    loop_guard:["Loop protection (STP)","Enable only when a cable loop is possible between switch ports."],
    iso_wifi24:["2.4 GHz client isolation","Stops direct traffic between clients on the same 2.4 GHz SSID."],
    iso_wifi5:["5 GHz client isolation","Stops direct traffic between clients on the same 5 GHz SSID."],
    country24:["2.4 GHz regulatory country","Choose the real operating country; the live driver channel list is enforced."],
    country5:["5 GHz regulatory country","Choose the real operating country; DFS and channel restrictions remain enforced."],
    radio0_enabled:["Enable 2.4 GHz radio","Turns the radio on or off without deleting its settings."],
    radio1_enabled:["Enable 5 GHz radio","Turns the radio on or off without deleting its settings."],
    txpower_radio0:["Requested 2.4 GHz TX power","Requested value only; the accepted runtime value is reported separately."],
    txpower_radio1:["Requested 5 GHz TX power","Requested value only; the accepted runtime value is reported separately."],
    txpower:["Requested TX power","Range 1-38 dBm; diagnostics show what the driver accepted."],
    program_mode:["Programming mode","Choose the network role; only fields required for that role are shown."],
    device_ip:["Device management IP","Primary LAN management address. The Wi-Fi-only rescue address remains separate."],
    vlan_id:["Management VLAN ID","Shown only in VLAN modes and applied through Safe Apply."],
    ssid:["Primary SSID","Network name used by the primary 2.4 and 5 GHz access points."],
    ch24:["2.4 GHz channel","Automatic or a channel currently allowed by the driver."],
    ch5:["5 GHz channel","Only live driver-supported channels are offered; DFS rules still apply."],
    htmode24:["2.4 GHz channel width","HE20 is the stable default; HE40 uses a wider channel."],
    htmode5:["5 GHz channel width","HE80 is the maximum configured width supported by this device."],
    mesh_id:["Mesh ID","Must match on both mesh units together with the 5 GHz channel and width."],
    mesh_role:["Mesh unit role","Select the wired sender/controller side or the receiving agent side."],
    wds_peer_ssid:["WDS peer SSID","SSID from which this receiver obtains its WDS backhaul."],
    hide_ssid:["Hide SSID","Controls whether the primary SSID is advertised."],
    security:["Wi-Fi security","Choose Open, WPA2, WPA3-SAE, or WPA2/WPA3 mixed mode."],
    wifi_password:["Wi-Fi password","Leave blank to retain the current key."],
    neighbor_enabled:["Show in MikroTik Neighbors","Advertises the device identity locally through MNDP on UDP 5678."],
    customer_name:["Customer name","Displayed in the local MikroTik Neighbors identity."],
    customer_device:["Customer device name","Friendly installation or location name for this access point."],
    customer_phone:["Customer phone","Optional phone field in the local MNDP identity."],
    neighbor_interval:["MikroTik Neighbors interval","Advertisement interval from 10 to 300 seconds."],
    delete_previous:["Explicit configuration takeover","Remove old external VLAN, Mesh, or WDS ownership only when selected."],
    pppoe_user:["PPPoE PAP/CHAP user","Used only in PPPoE access-point mode."],
    pppoe_pass:["PPPoE password","Leave blank to retain an existing password."],
    broadband_port:["Broadband input port","WAN is reserved for PPPoE; LAN1-LAN3 remain switch ports."],
    pppoe_vlan_enabled:["PPPoE WAN tagging","Use a tagged WAN only when required by the service provider."],
    pppoe_vlan_id:["PPPoE WAN VLAN ID","802.1Q tag from 1 to 4094 for the WAN service."],
    reset_button_enabled:["Reset button policy","Allows or blocks the product reset-button handler."],
    reset_mode:["Reset-button action","Choose factory reset, reboot only, or blocked."],
    nat_enabled:["NAT (automatic by mode)","Enabled for PPPoE routing and disabled for bridge AP modes."],
    dhcp_server:["DHCP server (automatic by mode)","Enabled for PPPoE routing and disabled for bridge AP modes."],
    firewall_enabled:["Firewall service","Controls the local firewall service through Safe Apply."],
    broadband_enabled:["Broadband state","Derived from the selected PPPoE mode."],
    fake_mesh:["WDS compatibility mode","Uses WDS only when the driver and peer support it."]
  };
  function localizeControlField(input) {
    if (state.lang !== "en" || !input) return input;
    var f = Object.assign({}, input), copy = CONTROL_EN[f.name];
    if (copy) { f.label = copy[0]; f.hint = copy[1]; }
    if (/^(?:sec_enabled|flood_guard|rogue_dhcp_guard|loop_guard|iso_wifi24|iso_wifi5|radio0_enabled|radio1_enabled|neighbor_enabled|reset_button_enabled|firewall_enabled)$/.test(f.name))
      f.options = "1:Enabled,0:Disabled";
    if (/^lan[1-3]_enabled$/.test(f.name)) f.options = "1:Enabled,0:Disabled";
    if (/^lan[1-3]_isolate$/.test(f.name)) f.options = "1:Isolated,0:Not isolated";
    if (/^lan[1-3]_vlan_mode$/.test(f.name)) f.options = "auto:Automatic (recommended),plain:No VLAN (plain switch),access:Access VLAN,trunk:Trunk VLAN";
    if (f.name === "program_mode") f.options = "ap:Access Point (AP),ap_vlan:AP + VLAN,mesh:Mesh,mesh_vlan:Mesh + VLAN,wds_sender:WDS sender,wds_sender_vlan:WDS sender + VLAN,wds_receiver:WDS receiver,wds_receiver_vlan:WDS receiver + VLAN,pppoe_ap:PPPoE Access Point";
    if (f.name === "hide_ssid") f.options = "0:Visible,1:Hidden";
    if (f.name === "security") f.options = "open:Open,wpa2:WPA2,wpa3:WPA3 SAE,mixed:WPA2/WPA3 mixed";
    if (f.name === "mesh_role") f.options = "sender:Sender (2.4 + 5 GHz AP),receiver:Receiver (2.4 GHz AP)";
    if (f.name === "delete_previous") f.options = "0:Preserve LuCI configuration,1:Clean up and take ownership";
    if (f.name === "reset_mode") f.options = "factory:Factory reset,reboot:Reboot only,disabled:Blocked";
    if (/^(?:nat_enabled|dhcp_server|broadband_enabled|fake_mesh)$/.test(f.name)) f.options = "0:Disabled,1:Enabled";
    return f;
  }
  function fieldHtml(f) {
    f = localizeControlField(f);
    ...
    if (type === "number" && f.options) {
      ...
      if (/^txpower(?:_radio[01])?$/.test(f.name)) {
        var inputIdBase = "txpower-" + String(f.name || "value").replace(/[^A-Za-z0-9_-]/g, "-");
        var hintId = inputIdBase + "-hint";
        var rangeLabel = state.lang === "ar" ? "\u0645\u0646\u0632\u0644\u0642 \u0637\u0627\u0642\u0629 \u0627\u0644\u0628\u062b" : "TX power slider";
        var numberLabel = state.lang === "ar" ? "\u0642\u064a\u0645\u0629 \u0637\u0627\u0642\u0629 \u0627\u0644\u0628\u062b \u0627\u0644\u062f\u0642\u064a\u0642\u0629" : "Exact TX power value";
        var presetLabel = state.lang === "ar" ? "\u0642\u064a\u0645 \u0637\u0627\u0642\u0629 \u0627\u0644\u0628\u062b \u0627\u0644\u062c\u0627\u0647\u0632\u0629" : "TX power presets";
        var presetDisabled = f.readonly ? " disabled" : "";
        var presets = [5, 10, 15, 20, 23, 26, 30, 35, 38].map(function (v) {
          var label = v + " dBm";
          return '<button type="button" class="txpower-preset latin" data-tx-preset="' + esc(f.name) + '" data-value="' + v + '"' + presetDisabled + '>' + label + '</button>';
        }).join("");
        return '<fieldset class="royal-field txpower-fieldset' + dsaClass + '" data-royal-group="' + esc(f.group || "") + '" data-modes="' + esc(f.modes || "") + '"' + dsaAttr + '><legend>' + esc(f.label) + '</legend>' +
          '<div class="txpower-control"><label class="txpower-input" for="' + inputIdBase + '-range"><span>' + esc(rangeLabel) + '</span><input id="' + inputIdBase + '-range" class="latin" data-tx-range="' + esc(f.name) + '" type="range" aria-describedby="' + hintId + '"' + at + ' value="' + esc(f.value) + '"' + ro + '></label>' +
          '<label class="txpower-input" for="' + inputIdBase + '-number"><span>' + esc(numberLabel) + '</span><input id="' + inputIdBase + '-number" class="latin" data-ctl-field="' + esc(f.name) + '" type="number" aria-describedby="' + hintId + '"' + at + ' value="' + esc(f.value) + '" inputmode="numeric" autocomplete="off"' + ro + '></label>' +
          '<div class="txpower-presets" role="group" aria-label="' + esc(presetLabel) + '">' + presets + '</div></div><small id="' + hintId + '">' + esc(f.hint || "") + '</small></fieldset>';
      }
  ...
  function renderWizardControl(section, data) {
    ...
    var actions = '<div class="branch-actions">' + (data.actions || []).filter(function (a) { return !a.url; }).map(function (a) {
      return '<button class="' + (a.confirm ? "btn" : "btn primary") + '" data-ctl-section="' + esc(section) + '" data-ctl-action="' + esc(a.id) + '" data-ctl-confirm="' + (a.confirm ? "1" : "") + '">' + esc(controlActionLabel(a)) + '</button>';
    }).join("") + '</div>';
    return cards +
  ...
  function updateWizardPreview() {
    ...
    var wanPreview = fv("program_mode") === "pppoe_ap"
      ? (fv("pppoe_vlan_enabled") === "1" ? "wan." + (fv("pppoe_vlan_id") || "?") + " (802.1Q)" : "Bare WAN (untagged)")
      : (state.lang === "ar" ? "محجوز / متوقف" : "Reserved / parked");
    patchHtml(box, '<div class="kv"><span>' + esc(tr("previewMode")) + '</span><b class="latin">' + esc(modeLabel || "-") + '</b></div>' +
      ...
      '<div class="kv"><span>' + esc(tr("previewWan")) + '</span><b class="latin">' + esc(wanPreview) + '</b></div>' +
  ...
  function syncWizardMode() {
    ...
    var wanTag = panel.querySelector('[data-ctl-field="pppoe_vlan_enabled"]');
    var wanVid = panel.querySelector('[data-ctl-field="pppoe_vlan_id"]');
    if (wanVid) {
      wanVid.disabled = !pppoe || !wanTag || wanTag.value !== "1";
      wanVid.setAttribute("aria-disabled", wanVid.disabled ? "true" : "false");
      if (typeof wanVid.setCustomValidity === "function") wanVid.setCustomValidity("");
    }
  ...
  function renderBranchDetail(groupId, item) {
    ...
    return '<div class="branch-detail" data-control-section="' + esc(section) + '">' +
      ...
      '</div>' +
      '<div id="' + ctlId + '" class="ctl-status" data-ui-preserve-children="1">Loading router controls...</div>' +
      '</div>';
  }
  ...
  function renderEvents(data) {
    ...
    var all = apiEvents.map(function (entry) {
      var value = String(entry == null ? "" : entry), base = "api:" + value;
      occurrences[base] = (occurrences[base] || 0) + 1;
      return { key:base + ":" + occurrences[base], text:value };
    }).concat(state.events.map(function (entry) {
      return { key:"page:" + entry.t + ":" + entry.msg, text:new Date(entry.t).toLocaleTimeString() + " " + entry.msg };
    })).slice(0, 60);
    return card("Events / Alerts", '<div class="events">' + (all.length ? all.map(function (entry) { return '<div class="event" data-ui-key="event:' + esc(entry.key) + '">' + esc(entry.text) + '</div>'; }).join("") : '<div class="empty">' + tr("emptyEvents") + '</div>') + '</div>', "filter", "bolt", "events");
  }
  function renderActions() {
    ...
    return sectionHead(tr("actions"), tr("actionsHint"), tr("safeAction")) +
      '<div class="actions">' + actions.map(function (a) { return '<button class="action" data-ui-key="action:' + a[0] + '" data-action="' + a[0] + '"><strong>' + esc(a[1]) + '</strong><span>' + esc(a[2]) + '</span></button>'; }).join("") + '</div>' + renderEvents(state.latest || {});
  }
  function renderIsolation() {
    return sectionHead(tr("isolation"), tr("isolationHint"), "Xiaomi CR6608") +
      ...
      '<div id="ctl_isolation" class="ctl-status" data-ui-preserve-children="1">' + esc(tr("loading")) + '</div>' +
      '</div>';
  }
  function renderQuick() {
    return sectionHead(tr("quick"), tr("quickHint"), "Xiaomi CR6608") +
      ...
      '<div id="ctl_wizard" class="ctl-status" data-ui-preserve-children="1">' + esc(tr("loading")) + '</div>' +
      '</div>';
  }
  // New-device join alert is session-only; no client inventory is persisted.
  function detectNewDevices(data) {
    var set = state.knownMacs || (state.knownMacs = {});
    var first = !Object.keys(set).length; // first sample: learn silently
    var devices = mergeDevices(data);
    var overflow = devices.length > MAX_PAGE_CLIENT_MACS;
    var suppressNewDeviceAlerts = first || overflow || state.clientInventoryOverflow;
    state.clientInventoryOverflow = overflow;
    devices.forEach(function (d) {
      var m = String(d.mac || "").replace(/^\s+|\s+$/g, "").toLowerCase();
      var wasKnown = !!m && Object.prototype.hasOwnProperty.call(set, m);
      m = touchPageClient(m);
      if (!m || wasKnown) return;
      set[m] = 1;
...
return H.card(A?'أبعد العملاء':'Distance Leaderboard',h,String(L.length)+(A?' جهاز':' clients'),'device')}},
  ...
  // #59 — bounded summary for this open page only. It is not a calendar report
  // and never survives a reload, logout, or authentication boundary.
  PRO_FEATURES.push({key:"weekly_report",ar:"ملخص الجلسة الحية",en:"Live Page Session",cat:"Automation & UX",fn:function(d,H){
    var A=H.lang==="ar", now=Date.now(), summary=state.liveSessionSummary;
    if(!summary){ summary={started:now,samples:0,peakClients:0,maxTemp:null}; state.liveSessionSummary=summary; }
    if(!state.suspendHistory){
      var cl=Math.max(Number(d.clients)||0,(H.mergeDevices(d)||[]).length),tc=H.num(d.temperature_c);
      summary.samples+=1;
      summary.peakClients=Math.max(summary.peakClients,cl);
      if(H.finite(tc))summary.maxTemp=summary.maxTemp===null?tc:Math.max(summary.maxTemp,tc);
    }
    var elapsed=Math.max(0,now-summary.started),minutes=Math.floor(elapsed/60000),seconds=Math.floor((elapsed%60000)/1000);
    var body="<div class='grid two'><div class='traffic-box'><span>"+(A?"مدة الصفحة":"page open")+"</span><b class='latin'>"+minutes+"m "+seconds+"s</b></div>"+
      "<div class='traffic-box'><span>"+(A?"العينات الحية":"live samples")+"</span><b class='latin'>"+summary.samples+"</b></div>"+
      "<div class='traffic-box'><span>"+(A?"ذروة العملاء":"peak clients")+"</span><b class='latin'>"+summary.peakClients+"</b></div>"+
      "<div class='traffic-box'><span>"+(A?"أقصى حرارة":"max temp")+"</span><b class='latin'>"+(summary.maxTemp===null?"—":H.fmt(summary.maxTemp,1)+"°C")+"</b></div></div>";
    body+="<small class='muted'>"+(A?"ذاكرة مؤقتة لهذه الصفحة فقط؛ تُمسح عند إعادة التحميل أو الخروج.":"Temporary memory for this open page only; cleared on reload or logout.")+"</small>";
    return H.card(A?"ملخص الجلسة الحية":"Live Page Session",body,summary.samples+" live","gear");
  }});
  ...
  PRO_FEATURES.push({key:"outage_log",ar:"انتقالات الوصلة الحية",en:"Live Uplink Transitions",cat:"Automation & UX",fn:function(d,H){
    ...
    var note="<small class='muted'>"+(A?'ذاكرة هذه الصفحة فقط؛ تُمسح عند إعادة التحميل أو الخروج، والفجوات غير معروفة.':'Open-page memory only; cleared on reload or logout, and sampling gaps are unknown.')+"</small>";
    return H.card(A?"انتقالات الوصلة الحية":"Live Uplink Transitions",cur+list+note,String(st.events.length),"net");
  }});
  // #62 — open-page dashboard fetch timing over observed samples.
  PRO_FEATURES.push({key:"latency_monitor",ar:"عينات جلب اللوحة الحية",en:"Live Dashboard Fetch Samples",cat:"Traffic & Bandwidth",fn:function(d,H){
    var A=H.lang==="ar", lt=H.num(d&&d.latency_ms);
    var hist=state.fetchSamples;
    if(!state.suspendHistory&&H.finite(lt)){ hist.push(lt); if(hist.length>60) hist.shift(); }
    if(!hist.length) return H.card(A?"عينات جلب اللوحة الحية":"Live Dashboard Fetch Samples","<div class='empty'>"+(A?"لا بيانات":"No data")+"</div>",null,"net");
    ...
    body+="<small class='muted'>"+(A?'آخر 60 عينة في ذاكرة الصفحة فقط؛ ليست RTT للشبكة وتُمسح عند الإغلاق.':'Last 60 samples in page memory only; not network RTT and cleared on close.')+"</small>";
    return H.card(A?"عينات جلب اللوحة الحية":"Live Dashboard Fetch Samples",body,H.fmt(now,0)+"ms","net");
  }});
  ...
  PRO_FEATURES.push({key:"perf_pack",ar:"حزمة الأداء الاحترافية",en:"Performance Pack",cat:"Automation & UX",fn:function(d,H){
    ...
    var body=head+
      grp(A?"MIMO / AX":"MIMO / AX",
        row(p.mu_bf_he,"DL MU-MIMO (AX)","DL MU-MIMO (AX)",A?"قدرة تنزيل مُعلنة؛ الجدولة تحتاج عدة عملاء متوافقين":"DL advertised; scheduling needs compatible clients")+
        row(p.mu_bf_vht,"DL MU-MIMO (AC)","DL MU-MIMO (AC)",A?"قدرة تنزيل مُعلنة لأجهزة AC؛ الاستخدام غير مقاس":"DL advertised for AC; use not measured")+
  ...
  PRO_FEATURES.push({key:"mimo_matrix",ar:"مصفوفة AX/AC/N",en:"AX/AC/N Matrix",cat:"Automation & UX",fn:function(d,H){
    ...
    var ulState=String(p.ul_muru_guard_state||p.ul_muru_state||"unknown"), ulGuardHealthy=p.ul_muru_guard_healthy===true;
    var ulMask=H.num(p.ul_muru_mask_module), ulRequest=String(p.ul_muru_host_request||"unknown");
    var ulAirtime=String(p.ul_muru_airtime_evidence||"unverified"), ulOta=String(p.ul_muru_ota_evidence||"not-established");
    var ulArmed=ulRequest==="mask15-guarded"&&ulMask===15&&ulState==="armed"&&ulGuardHealthy&&ulAirtime==="unverified"&&ulOta==="not-established";
    var ulLabel=ulArmed?(A?"mask15 · \u0645\u062d\u0645\u064a · \u0627\u0644\u0627\u0633\u062a\u062e\u062f\u0627\u0645 \u0627\u0644\u0647\u0648\u0627\u0626\u064a \u063a\u064a\u0631 \u0645\u062b\u0628\u062a":"mask15 · guarded · airtime unverified"):(ulRequest==="mask15-guard-unhealthy"?"mask15 · guard stale/stopped · airtime unverified":(ulRequest==="mask0-fault-disabled"?"mask0 · fault disabled":(ulRequest==="mask0-guard-unhealthy"?"mask0 · guard state missing/invalid":(ulRequest==="mask0-disabled"?"mask0 · disabled":"unknown · not armed"))));
    ...
    var h="<div class='table-wrap'><table style='font-size:11px'>"+tableCaption(t)+"<thead><tr>"+tableHeaderCells([A?"\u0627\u0644\u0645\u064a\u0632\u0629":"Feature",cols[0][0],cols[1][0],cols[2][0]])+"</tr></thead><tbody>";
    rows.forEach(function(r){ h+="<tr><th scope='row' style='font-weight:700;color:var(--text);letter-spacing:0;text-transform:none'>"+H.esc(r[0])+"</th>"; for(var i=1;i<4;i++){ var v=r[i], ok=v.indexOf("✓")===0; h+="<td class='latin' style='color:"+(ok?"var(--excellent)":v==="—"?"var(--muted)":"var(--text)")+"'>"+H.esc(v)+"</td>"; } h+="</tr>"; });
    h+="</tbody></table></div><p class='muted' style='margin-top:6px;font-size:11px'>"+(A?"\u062a\u064f\u0639\u0631\u0636 \u062d\u0627\u0644\u0629 \u0637\u0644\u0628 UL \u0645\u0646 \u0642\u0646\u0627\u0639 MURU \u0648\u062d\u0627\u0644\u0629 \u0627\u0644\u062d\u0627\u0631\u0633 \u0628\u0634\u0643\u0644 \u0645\u0646\u0641\u0635\u0644. mask15 \u064a\u0639\u0646\u064a \u0623\u0646 \u0628\u062a\u0627\u062a \u0627\u0644\u0637\u0644\u0628 \u0627\u0644\u0623\u0631\u0628\u0639\u0629 \u0645\u0647\u064a\u0623\u0629 \u062a\u062d\u062a \u062d\u0645\u0627\u064a\u0629 \u0627\u0644\u0627\u0633\u062a\u0639\u0627\u062f\u0629\u061b \u0648\u0644\u0627 \u064a\u062b\u0628\u062a \u062a\u0637\u0628\u064a\u0642 Firmware \u0623\u0648 \u0627\u0644\u062c\u062f\u0648\u0644\u0629 \u0627\u0644\u0647\u0648\u0627\u0626\u064a\u0629 \u0623\u0648 OTA \u0623\u0648 \u0627\u0644\u062c\u0627\u0647\u0632\u064a\u0629 \u0644\u0644\u0628\u064a\u0639.":"The UL host request is reported from the MURU mask and guard separately. mask15 means all four request bits are armed under recovery protection; it is not firmware-apply, airtime, OTA, or sale-readiness proof.")+"</p>";
  ...
  function renderProInsights(data) {
    ...
    out += '<div class="insight-summary">' +
      ...
      '</div>';
    var selectedIndex = Math.max(0, ordered.indexOf(selected));
    out += '<div class="insight-tabs" role="tablist" aria-label="' + esc(state.lang === "ar" ? "\u0641\u0626\u0627\u062a \u0627\u0644\u0645\u0624\u0634\u0631\u0627\u062a" : "Insight categories") + '">' + ordered.map(function (category, index) {
      var label = state.lang === "ar" ? (PRO_CAT_AR[category] || category) : category;
      return '<button id="insight-tab-' + index + '" data-ui-key="insight-tab:' + esc(category) + '" type="button" role="tab" aria-selected="' + (category === selected ? "true" : "false") + '" aria-controls="insight-panel" tabindex="' + (category === selected ? "0" : "-1") + '" class="' + (category === selected ? "active" : "") + '" data-insight-category="' + esc(category) + '"><span>' + esc(label) + '</span><b>' + cats[category].length + '</b></button>';
    }).join("") + '</div>';
    var seen = {};
    var selectedLabel = state.lang === "ar" ? (PRO_CAT_AR[selected] || selected) : selected;
    out += '<div id="insight-panel" role="tabpanel" aria-labelledby="insight-tab-' + selectedIndex + '" tabindex="0"><div class="insight-category-head"><h3>' + esc(selectedLabel) + '</h3><span>' + cats[selected].length + '</span></div><div class="grid pro-grid">';
    cats[selected].forEach(function (f) {
      if (seen[f.key]) return; seen[f.key] = 1;
      var html = "";
      try { html = f.fn(data, H); } catch (e) { html = ""; }
      if (html) out += '<div data-ui-key="insight:' + esc(f.key) + '" style="display:contents">' + html + '</div>';
    });
    return out + '</div></div>';
    } finally {
      state.suspendHistory = priorSuspendHistory;
  ...
  function renderFactoryIdentity(data) {
    var identity = data.mac_identity || {}, roles = identity.roles || {};
    var ok = identity.ok === true && identity.status === "PASS";
    var ar = state.lang === "ar";
    var labels = ar
      ? { title:"\u0647\u0648\u064a\u0627\u062a \u0627\u0644\u0645\u0635\u0646\u0639", lan:"LAN / \u0627\u0644\u062c\u0633\u0631", wan:"WAN", wifi24:"Wi-Fi 2.4G", wifi5:"Wi-Fi 5G", source:"\u0627\u0644\u0645\u0635\u062f\u0631", dsa:"LAN1\u2013LAN3 \u0645\u0646\u0627\u0641\u0630 DSA \u0641\u064a\u0632\u064a\u0627\u0626\u064a\u0629 \u062a\u0631\u062b MAC \u0627\u0644\u062c\u0633\u0631 \u0645\u0646 GMAC0 \u062d\u0633\u0628 \u0627\u0644\u062a\u0635\u0645\u064a\u0645\u061b \u0647\u0630\u0627 \u0644\u064a\u0633 \u062a\u0635\u0627\u062f\u0645\u0627\u064b.", pass:"\u0645\u0637\u0627\u0628\u0642\u0629", fail:"\u0641\u0634\u0644 \u0627\u0644\u062a\u062d\u0642\u0642" }
      : { title:"Factory identities", lan:"LAN / bridge", wan:"WAN", wifi24:"Wi-Fi 2.4G", wifi5:"Wi-Fi 5G", source:"Source", dsa:"LAN1\u2013LAN3 are physical DSA ports that inherit the GMAC0 bridge MAC by design; this is not a collision.", pass:"Verified", fail:"Verification failed" };
    var roleOrder = [["lan", labels.lan], ["wan", labels.wan], ["wifi24", labels.wifi24], ["wifi5", labels.wifi5]];
    var rows = roleOrder.map(function (entry) {
      var value = roles[entry[0]] || "\u2014";
      return '<div><span>' + esc(entry[1]) + '</span><b class="latin identity-mac">' + esc(String(value).toUpperCase()) + '</b></div>';
    }).join("");
    rows += '<div><span>' + esc(labels.source) + '</span><b>Factory / NVMEM</b></div>';
    var body = '<div class="kv factory-identity-list">' + rows + '</div>' +
      '<p class="identity-dsa-note">' + esc(labels.dsa) + '</p>' +
      (!ok && identity.reason ? '<p class="ctl-note latin">' + esc(identity.reason) + '</p>' : '');
    return card(labels.title, body, ok ? labels.pass : labels.fail, ok ? "shield" : "warn");
  }
  function renderOverviewLite(data, rates) {
    ...
    return sectionHead(tr("overview"), tr("subtitle"), nowTime()) + '<div class="grid two">' +
      ...
      renderFactoryIdentity(data) + renderEvents(data) + '</div>';
  }
  function renderLiveSection(id, data, rates, force) {
    ...
    var focusedInsightCategory = "";
    if (id === "insights" && $("insights") && document.activeElement && $("insights").contains(document.activeElement))
      focusedInsightCategory = document.activeElement.dataset.insightCategory || "";
    if (id === "overview") patchHtml($("overview"), renderOverviewLite(data, rates));
    else if (id === "network") patchHtml($("network"), renderNetwork(data, rates));
    else if (id === "devices") patchHtml($("devices"), renderDevices(data));
    else if (id === "wifi") patchHtml($("wifi"), renderWifi(data));
    else if (id === "insights" && $("insights") && (!data.lite || !$("insights").innerHTML)) patchHtml($("insights"), renderProInsights(data));
    else if (id === "system") patchHtml($("system"), renderSystem(data));
    else if (id === "actions") patchHtml($("actions"), renderActions());
    var root = $(id);
    if (root) {
      root.dataset.liveState = "live";
      if ((document.body.dataset.activeSection || "overview") === id) document.body.dataset.liveState = "live";
      flushScheduledCharts(root);
      bindDynamic(root);
      if (focusedInsightCategory) {
        Array.prototype.some.call(root.querySelectorAll("[data-insight-category]"), function (tab) {
          if (tab.dataset.insightCategory !== focusedInsightCategory) return false;
  ...
  function render(data, forceLiveSection, repaintOnly) {
    var staleSnapshot = snapshotIsStale(data);
    if (staleSnapshot) {
      markLiveSnapshotUnavailable();
      return;
    }
    var liveCounters = !staleSnapshot || data._liveLite === true;
    var recordLiveCounters = liveCounters && repaintOnly !== true;
    var priorSuspendHistory = state.suspendHistory;
    var priorSuspendAllHistory = state.suspendAllHistory;
    state.suspendHistory = staleSnapshot || repaintOnly === true;
    state.suspendAllHistory = repaintOnly === true;
    try {
    state.liveDataUnavailable = false;
    state.latest = data;
    window.__lastApi = data;
    if (!staleSnapshot && repaintOnly !== true) detectNewDevices(data);
    var rates = trafficRates(data, recordLiveCounters);
    ...
    if (repaintOnly !== true) updateAvailability(!!data.ok);
    pushHistory("latency", state.lastLatency || 0, 60, recordLiveCounters);
    pushHistory("rx", rates.rx, 60, recordLiveCounters); pushHistory("tx", rates.tx, 60, recordLiveCounters);
    renderKpis(data, rates, recordLiveCounters);
    var transportUnavailable = data._transportUnavailable === true;
    var connectionText = data.ok ? tr("online") : tr("offline");
    if (transportUnavailable) {
      connectionText = state.lang === "ar"
        ? "\u0627\u0644\u0628\u064a\u0627\u0646\u0627\u062a \u0627\u0644\u062d\u064a\u0629 \u063a\u064a\u0631 \u0645\u062a\u0627\u062d\u0629"
    ...
    if ($("snapshotWarning")) {
      ...
      $("snapshotWarning").textContent = staleSnapshot
        ...
        : "";
      if (staleSnapshot && transportUnavailable) {
        $("snapshotWarning").textContent = state.lang === "ar"
          ? "\u062a\u0639\u0630\u0631 \u062c\u0644\u0628 \u0628\u064a\u0627\u0646\u0627\u062a \u062d\u064a\u0629. \u0622\u062e\u0631 \u0644\u0642\u0637\u0629 \u0645\u0624\u0643\u062f\u0629 \u0645\u0646\u0630 " + uptime(Math.max(0, num(data.snapshot_age_s) || 0)) + " \u0648\u0644\u0627 \u062a\u064f\u0639\u0631\u0636 \u0643\u062d\u0627\u0644\u0629 \u0622\u0646\u064a\u0629."
          : "Live data could not be fetched. The last confirmed snapshot is " + uptime(Math.max(0, num(data.snapshot_age_s) || 0)) + " old and is not presented as current.";
      }
    }
    ...
    scheduleActiveControlLoad();
      ...
      state.suspendHistory = priorSuspendHistory;
      state.suspendAllHistory = priorSuspendAllHistory;
    }
  }
  function isLiveDataSection(id) {
    return /^(overview|network|devices|wifi|insights|system|actions)$/.test(String(id || ""));
  }
  function liveSectionUnavailableHeading(id) {
    var headings = {
      overview:tr("overview"), network:tr("networkTitle"), devices:tr("devices"),
      wifi:"WiFi AX / AC / N", insights:tr("insights"), system:tr("systemTitle"), actions:tr("actions")
    };
    return headings[id] || tr("unavailable");
  }
  function paintUnavailableKpis() {
    if (!$("kpiBar")) return;
    var keys = ["uptime","model","firmware","internet","devices","w24","w5","updated"];
    patchHtml($("kpiBar"), keys.map(function (key) {
      return '<div class="kpi unavailable" data-ui-key="kpi:' + key + '"><span>' + esc(key === "updated" ? tr("updated") : key) + '</span><b>—</b></div>';
    }).join(""));
  }
  function paintLiveUnavailableSection(id) {
    if (!isLiveDataSection(id) || !$(id)) return;
    patchHtml($(id), sectionHead(liveSectionUnavailableHeading(id), tr("subtitle"), tr("unavailable")) +
      '<article class="card live-unavailable" data-ui-key="live-unavailable:' + esc(id) + '"><div class="empty" role="status">' + esc(state.lang === "ar"
      ? "\u0627\u0644\u0628\u064a\u0627\u0646\u0627\u062a \u0627\u0644\u062d\u064a\u0629 \u063a\u064a\u0631 \u0645\u062a\u0627\u062d\u0629 \u0627\u0644\u0622\u0646\u061b \u0644\u0627 \u062a\u064f\u0639\u0631\u0636 \u0623\u064a \u0644\u0642\u0637\u0629 \u0633\u0627\u0628\u0642\u0629."
      : "Live data is unavailable; no previous snapshot is displayed.") + '</div></article>');
    $(id).dataset.liveState = "unavailable";
  }
  function markLiveSnapshotUnavailable() {
    // Fail closed: once the live read is unavailable, remove the prior values
    // instead of leaving an old snapshot visible with a warning badge.
    state.latest = null;
    state.liveDataUnavailable = true;
    window.__lastApi = null;
    state.previousTraffic = null;
    state.previousAt = 0;
    updateAvailability(false);
    paintUnavailableKpis();
    ["overview","network","devices","wifi","insights","system","actions"].forEach(paintLiveUnavailableSection);
    document.body.dataset.liveState = "unavailable";
    if ($("connectionState")) $("connectionState").textContent = state.lang === "ar" ? "البيانات الحية غير متاحة" : "Live data unavailable";
    if ($("connectionChip")) {
      $("connectionChip").classList.remove("ok", "warn");
      $("connectionChip").classList.add("bad");
    }
    if ($("statusPulse")) {
      $("statusPulse").classList.remove("warn");
      $("statusPulse").classList.add("bad");
    }
    if ($("snapshotWarning")) {
      $("snapshotWarning").hidden = false;
      $("snapshotWarning").textContent = state.lang === "ar"
        ? "البيانات الحية غير متاحة. حُذفت القيم السابقة من العرض وتجري إعادة المحاولة تلقائياً."
        : "Live data is unavailable. Previous values were removed from view; retrying automatically.";
    }
    if ($("sideStatus")) $("sideStatus").textContent = state.lang === "ar" ? "بانتظار عينة حية" : "waiting for live sample";
  }
  async function loadData(forceFull, refreshReason) {
    if (!state.session || state.navigationPending) return;
    if (forceFull === true) state.fullRefreshRequested = true;
    if (state._pollBusy) {
      // Keep the single in-flight live read and coalesce a forced request into
      // the one replaceable trailing refresh. Aborting a healthy CGI read made
      // rapid section changes surface browser request failures and wasted the
      // collector work that was already in progress.
      return;
    }
    ...
    try {
      // Bootstrap with procfs-only counters so the first phone viewport appears
      // immediately. Every later poll requests a live-only full endpoint. The
      // backend may coalesce only a just-completed sample and never returns an
      // older detailed snapshot to this interface.
      var bootstrapLite = !state.latest && !requestedFull;
      var full = requestedFull || !bootstrapLite;
      countDataRequest(1);
      requestCounted = true;
      var res = await fetchWithTimeout(authUrl(full ? API + "?live=1" : API + "?lite=1&live=1"), { credentials:"same-origin", cache:"no-store", headers:authHeaders(), signal: ctrl ? ctrl.signal : undefined }, 15000);
      ...
      if (!isLite && (staleSnapshot || data.snapshot_live !== true)) {
        var staleError = new Error("live snapshot unavailable");
        staleError.code = staleSnapshot ? "SMARTAP_STALE_SNAPSHOT" : "SMARTAP_LIVE_SNAPSHOT_REQUIRED";
        throw staleError;
      }
      if (!staleSnapshot) state.lastFreshAt = Date.now();
      if (isLite) {
        data._liveLite = true;
      ...
      render(data);
      if (bootstrapLite) scheduleDataRefresh(0, true, "bootstrap-full");
    } catch (e) {
      ...
      // failures; these transient events remain in page memory only.
      ...
      if (requestCounted) countDataRequest(-1);
      ...
      // dropped. A failed full read also retries independently of the selected
      // polling interval, with a short bounded backoff to avoid a busy loop.
      if (state.fullRefreshRequested && state.session && !state.navigationPending) {
        var retryDelay = requestedFull && !superseded
          ? Math.min(3000, Math.max(1, state.apiFails) * 750) : 0;
        // An interval/navigation read may already consume the flag first. The
        // one replaceable timer gives rapid taps and stale retries latest-wins
        // semantics while the busy gate keeps the live GET count at one.
        scheduleDataRefresh(retryDelay, false, refreshReason || "trailing-full");
      }
    }
  }
  async function measureLocalSpeed(onProgress) {
    var pings = [];
    for (var i = 0; i < 5; i++) {
      var t0 = performance.now();
      var r = await fetchWithTimeout(authUrl(API + "?lite=1&probe=1&live=1&speedping=" + i), { credentials:"same-origin", cache:"no-store", headers:authHeaders() }, API_TIMEOUT_MS);
      if (r.status === 403) { requireLogin(tr("sessionExpired")); return; }
      if (!r.ok) throw new Error("HTTP " + r.status);
      await r.text();
      pings.push(performance.now() - t0);
    }
    var bytesRead = 0, transferStart = performance.now();
    for (var n = 0; n < 3; n++) {
      var payload = await fetchWithTimeout(authUrl(API + "?speedtest=1&nonce=" + Date.now() + "-" + n), { credentials:"same-origin", cache:"no-store", headers:authHeaders() }, 30000);
      if (payload.status === 403) { requireLogin(tr("sessionExpired")); return; }
      if (!payload.ok) throw new Error("HTTP " + payload.status);
      bytesRead += (await payload.arrayBuffer()).byteLength;
      if (typeof onProgress === "function") {
        var elapsed = Math.max(1, performance.now() - transferStart);
        onProgress(bytesRead * 8 / (elapsed / 1000) / 1000000);
      }
    }
    var ms = performance.now() - transferStart, avg = pings.reduce(function (a,b) { return a+b; },0) / pings.length;
    var jitter = Math.sqrt(pings.map(function (x) { return Math.pow(x - avg, 2); }).reduce(function (a,b) { return a+b; },0) / pings.length);
    return { latencyMs:avg, jitterMs:jitter, bytesPerSecond:bytesRead / Math.max(ms / 1000, 0.001) };
  }
  function beginSpeedTest(button) {
    if (state.speedTestBusy) return false;
  ...
  function finishSpeedTest(button) {
  ...
  async function speedTest(button) {
    if (!beginSpeedTest(button)) return;
    try {
      var result = await measureLocalSpeed();
      if (!result) return;
      toast(tr("localTest") + ": " + fmt(result.latencyMs,0) + "ms, " +
        (state.lang === "ar" ? "تذبذب " : "jitter ") + fmt(result.jitterMs,0) + "ms, " +
        bps(result.bytesPerSecond));
    } catch (e) {
      toast((state.lang === "ar" ? "فشل الاختبار: " : "Test failed: ") + e.message);
    } finally {
      finishSpeedTest(button);
    }
  }
  async function action(name, button) {
    if (name === "refresh") return scheduleDataRefresh(0, true, "action-refresh");
    if (name === "speedtest") return speedTest(button);
    ...
    try {
      ...
      var j = result.data;
      if (!j || j.ok !== true) {
        toast((j && (j.message || j.summary || j.error)) || (state.lang === "ar" ? "فشل الإجراء" : "Action failed"));
        // Some backends can fail after a partial mutation.  Re-read the whole
        // state, but never record a failed action as a successful UI event.
  ...
  function clearControlDirty(section) {
    var panel = document.querySelector('[data-control-section="' + section + '"]');
    Array.prototype.forEach.call(panel ? panel.querySelectorAll("[data-ctl-field][data-ui-dirty]") : [], function (field) {
      field.removeAttribute("data-ui-dirty");
    });
  }
  function validateWizardWanVlan(section, actionName) {
    if (section !== "wizard" || !/^(save_royal|apply_royal)$/.test(actionName || "")) return true;
    var panel = document.querySelector('[data-control-section="wizard"]');
    if (!panel) return false;
    var mode = panel.querySelector('[data-ctl-field="program_mode"]');
    var enabled = panel.querySelector('[data-ctl-field="pppoe_vlan_enabled"]');
    var vid = panel.querySelector('[data-ctl-field="pppoe_vlan_id"]');
    if (!mode || mode.value !== "pppoe_ap" || !enabled || enabled.value !== "1") return true;
    var value = vid ? String(vid.value || "") : "";
    var valid = /^[0-9]+$/.test(value) && Number(value) >= 1 && Number(value) <= 4094;
    if (valid) return true;
    var message = state.lang === "ar" ? "رقم VLAN لمنفذ PPPoE WAN يجب أن يكون من 1 إلى 4094." : "PPPoE WAN VLAN ID must be an integer from 1 to 4094.";
    if (vid) {
      if (typeof vid.setCustomValidity === "function") vid.setCustomValidity(message);
      if (typeof vid.reportValidity === "function") vid.reportValidity();
      if (typeof vid.focus === "function") vid.focus();
    }
    toast(message);
    return false;
  }
  ...
  async function loadControl(section, actionName, params, button) {
    ...
    if (!actionName && state.postLock) {
      delete box.dataset.loaded;
      if (activeControlSection() === section) scheduleActiveControlLoad(250);
      return;
    }
    ...
    box.dataset.loaded = "1";
    var hasMountedControl = !!(box.querySelector && box.querySelector("[data-ctl-field],[data-ctl-action]"));
    if (!actionName && !hasMountedControl) {
      box.className = "ctl-status";
      box.textContent = state.lang === "ar" ? "جارٍ تحميل عناصر التحكم..." : "Loading router controls...";
    } else if (!actionName) {
      paintControlNotice(box, state.lang === "ar" ? "جارٍ تحديث عناصر التحكم..." : "Refreshing router controls...", false);
    }
    ...
    try {
      ...
      if (res.status === 403) return requireLogin(tr("sessionExpired"));
      if (!actionName && (res.ok !== true || !data || data.ok !== true)) {
        var readError = new Error(data && data.error ? String(data.error) : "control-read-failed");
        readError.code = "SMARTAP_CONTROL_READ";
        throw readError;
      }
      ...
      if (actionName && data && data.ok === false) {
        ...
        if (actionName === "keep_changes" || actionName === "rollback_last") {
          // Keep the still-armed transaction actionable after a rejected
      ...
      if (actionName && data && data.ok === true && controlActionAffectsSnapshot(actionName))
        clearControlDirty(section);
      ...
      box.className = "";
      stopProgress();
      patchHtml(box, renderControlData(section, data));
      if (section === "wizard") syncWizardMode();
      if (actionName && data.summary) toast(data.summary);
      bindDynamic(box);
      // A transport-successful response is not a successful control read until
      // its payload has rendered and bound without error. Committing the cache
      // and clearing backoff earlier would turn malformed payloads into an
      // unlimited 750 ms retry loop and preserve the bad payload for rebuilds.
      state.controlCache[section] = data;
      if (!actionName) resetControlReadRetry(section);
    } catch (e) {
      stopProgress();
      var ownsPassiveRead = !actionName && state.controlReadGeneration === generation &&
        state.controlReadBox === box;
      if (!isControlRequestCurrent(generation, section, box) || (!actionName && isAbortError(e))) {
        // Do not let a stale completion erase a newer read for the same panel.
        // If this request still owns the active panel, release and retry after
        // any already-started router CGI workers have had time to drain.
        if (ownsPassiveRead) {
          delete box.dataset.loaded;
          if (activeControlSection() === section)
            scheduleControlReadRetry(section);
        } else if (activeControlSection() !== section) {
          delete box.dataset.loaded;
        }
        return;
      }
      ...
      if (actionName === "apply_isolation" && section === "isolation") {
        ...
        paintControlNotice(box, state.lang === "ar" ? "يُعاد تحميل الشبكة، جارٍ تحديث حالة المنافذ…" : "Network is reloading; refreshing port status…", false);
        queueControlReloadIfActive(section, box, 4500);
        return;
      }
      if (!actionName && activeControlSection() === section)
        scheduleControlReadRetry(section);
      var controlError = (state.lang === "ar" ? "خطأ في واجهة التحكم: " : "Control API error: ") + e.message;
      if (hasMountedControl || actionName) paintControlNotice(box, controlError, true);
      else { box.className = "ctl-status"; box.textContent = controlError; }
    } finally {
      stopProgress();
      if (!actionName && state.controlReadGeneration === generation && state.controlReadBox === box) {
        state.controlReadController = null;
        state.controlReadBox = null;
        state.controlReadGeneration = 0;
      }
  ...
  function queueControlReloadIfActive(section, box, delayMs) {
    if (box && box.dataset) delete box.dataset.loaded;
    if (!box || box.isConnected === false || activeControlSection() !== section) return false;
    resetControlReadRetry(section);
    scheduleActiveControlLoad(delayMs);
    return true;
  }
  function scheduleActiveControlLoad(delayMs, allowExhausted) {
    var section = activeControlSection();
    if (!section) return;
    var box = $("ctl_" + sid(section));
    if (!box || box.dataset.loaded) return;
    if (allowExhausted !== true && state.controlReadFailureSection === section &&
        state.controlReadFailureCount >= CONTROL_READ_MAX_RETRIES) return;
    // One pending timer owns the next passive read. Background telemetry paints
    // must not shorten a navigation debounce or an error/post-lock backoff.
    if (state.controlLoadTimer) return;
    var delay = Number(delayMs);
    if (!isFinite(delay) || delay < 0) delay = CONTROL_NAV_DEBOUNCE_MS;
    state.controlLoadTimer = setTimeout(function () {
      state.controlLoadTimer = 0;
      loadActiveControl();
    }, delay);
  }
  function nextControlReadRetryDelay(section) {
    if (state.controlReadFailureSection !== section) {
      state.controlReadFailureSection = section;
      state.controlReadFailureCount = 0;
    }
    if (state.controlReadFailureCount >= CONTROL_READ_MAX_RETRIES) return null;
    state.controlReadFailureCount++;
    return CONTROL_READ_RETRY_MS * Math.pow(2, state.controlReadFailureCount - 1);
  }
  function scheduleControlReadRetry(section) {
    var delay = nextControlReadRetryDelay(section);
    if (delay === null) return false;
    scheduleActiveControlLoad(delay, true);
    return true;
  }
  function resetControlReadRetry(section) {
    if (state.controlReadFailureSection !== section) return;
    state.controlReadFailureSection = "";
    state.controlReadFailureCount = 0;
  }
  ...
  function bindDynamic(rootNode) {
    ...
    function activateInsightTab(button) {
      var focusId = button.id;
      state.insightCategory = button.dataset.insightCategory || "System & Health";
      if (state.latest && $("insights")) {
        patchHtml($("insights"), renderProInsights(state.latest));
        bindDynamic($("insights"));
        var replacement = focusId ? $(focusId) : null;
        if (replacement) replacement.focus();
      }
    }
    Array.prototype.forEach.call(root.querySelectorAll("[data-insight-category]"), function (button) {
      button.onclick = function () { activateInsightTab(button); };
      bindTabKeyboard(button, "[data-insight-category]", activateInsightTab);
    });
    ...
    Array.prototype.forEach.call(root.querySelectorAll("[data-admin-group]"), function (b) {
      b.onclick = function () {
        ...
        if (panel) patchHtml(panel, renderBranchDetail(groupId, group.items[idx]));
    ...
    Array.prototype.forEach.call(root.querySelectorAll("[data-ctl-refresh]"), function (b) {
      b.onclick = function () {
        resetControlReadRetry(b.dataset.ctlRefresh);
        loadControl(b.dataset.ctlRefresh);
      };
    });
    Array.prototype.forEach.call(root.querySelectorAll("[data-ctl-action]"), function (b) {
      b.onclick = function () {
        ...
        if (!validateWizardWanVlan(section, actionName)) return;
      ...
      b.onclick = async function () {
        ...
        if (act === "__limit") { // bandwidth-hog "limit" -> ask Mbps, set per-client QoS
          ...
          try {
            ...
            var jl = result.data;
            if (!jl || jl.ok !== true) {
              toast((jl && (jl.summary || jl.text || jl.error)) || (state.lang === "ar" ? "فشل تطبيق الحد" : "Limit update failed"));
              queueFullRefresh(250);
              return;
            }
            toast(jl.summary || tr("ok")); event("Limit " + mac + " " + mb + "Mbps");
            queueFullRefresh(250);
    ...
    if (stBtn) stBtn.onclick = async function () {
      var old = stBtn.textContent; stBtn.textContent = tr("loading") + "...";
      try {
        var result = await postJsonLocked(CTL, { credentials:"same-origin", cache:"no-store",
          headers:authHeaders({ "Content-Type":"application/x-www-form-urlencoded" }),
        ...
        toast("selftest: " + e.message);
      } finally {
        if (stBtn.isConnected !== false) {
          stBtn.disabled = false;
          stBtn.textContent = old;
        }
      }
    };
    Array.prototype.forEach.call(root.querySelectorAll("[data-steer-mac]"), function (b) {
      b.onclick = async function () {
        ...
        try {
          ...
          var j = result.data;
          if (!j || j.ok !== true) {
            toast((j && (j.summary || j.text || j.error)) || (state.lang === "ar" ? "فشل توجيه العميل" : "Client steering failed"));
            queueFullRefresh(250);
            return;
          }
          toast(j.summary || tr("ok"));
  ...
  function pollLoop() {
    if (!state.session || document.hidden || state.navigationPending) return;
    clearTimeout(state.timer);
    state.timer = 0;
    Promise.resolve(loadData(snapshotIsStale(state.latest))).then(function () {
      if (!state.session || document.hidden || state.navigationPending) return;
      state.timer = setTimeout(pollLoop, state.interval * 1000);
    });
  }
  ...
  function quiesceDataReadsForNavigation() {
    state.navigationPending = true;
    clearTimeout(state.timer);
    state.timer = 0;
    clearStaleRetry();
    // LuCI navigation takes priority over a replaceable live GET. Abort it now,
    // keep every poll/reload gate closed, and only wait briefly for fetch.finally
    // to release its accounting. A stale browser counter must never hold the
    // settings button for the full API timeout.
    cancelDataRead();
    return new Promise(function (resolve) {
      var deadline = Date.now() + 1500;
      function check() {
        if (state.activeDataRequests === 0 && !state._pollBusy) return resolve();
  ...
  function retranslateControlSection(active) {
    ...
    patchHtml(container, active === "quick" ? renderQuick() : renderIsolation());
    container.dataset.uiVersion = UI_VERSION;
    var keep = container.querySelector('[data-ctl-action="keep_changes"]');
    var rollback = container.querySelector('[data-ctl-action="rollback_last"]');
    if (keep || rollback) {
      // Pending Safe Apply owns this subtree. Translate its mounted buttons in
      // place without probing, cancelling, or replacing the recovery state.
      if (keep) keep.textContent = state.lang === "ar" ? "تثبيت التغييرات" : "Keep changes";
      if (rollback) rollback.textContent = state.lang === "ar" ? "رجوع الآن" : "Rollback now";
      bindDynamic(container);
      return;
    }
    var box = $(section === "wizard" ? "ctl_wizard" : "ctl_isolation");
    var cached = state.controlCache[section];
    if (box && cached) {
      // Re-render translated copy from the already loaded response. The keyed
      // reconciler retains dirty controls, focus/caret, and the selected tab.
      patchHtml(box, renderControlData(section, cached), { preserveTabs:true });
      if (section === "wizard") syncWizardMode();
      bindDynamic(box);
    } else {
      bindDynamic(container);
  ...
  function setLanguage(lang) {
    ...
    renderChrome();
    updateNavSearchCopy();
    if ($("navSearch")) applyNavSearchFilter($("navSearch").value);
    ...
    if (/^(quick|isolation)$/.test(active)) {
      ...
      render(state.latest, true, true);
    } else if (state.liveDataUnavailable) {
      paintLiveUnavailableSection(active);
    }
  }
  function init() {
    var loggedOut = false;
    ...
    // Password-manager data is outside HTTP cache control.  The page therefore
    // treats the credential field as one-use input and destroys any value a
    // browser restored before DOMContentLoaded or from bfcache.
    clearLoginCredentialInput();
    renderChrome();
    setLoginText();
    function markControlDraft(event) {
      var field = event.target && event.target.closest ? event.target.closest("[data-ctl-field]") : null;
      if (field) field.dataset.uiDirty = "1";
    }
    document.addEventListener("input", markControlDraft);
    document.addEventListener("change", markControlDraft);
    if ($("loginForm")) {
    ...
    $("themeBtn").onclick = function () {
      ...
      if (act) redrawMountedCharts($(act));
    };
    $("intervalSelect").onchange = function () { state.interval = validPollInterval(this.value); startPolling(); };
    ...
    if ($("openWrtBtn")) $("openWrtBtn").onclick = async function () {
      ...
      try {
        await quiesceDataReadsForNavigation();
        await ensureLuciSession();
        window.location.assign("/cgi-bin/luci/admin/network/wireless");
      } catch (e) {
        state.navigationPending = false;
        if (state.session) startPolling();
        if (e && (e.status === 401 || e.status === 403 || e.status === 409)) requireLogin(tr("sessionExpired"));
        else toast(tr("openWrtAuthFailed"));
        button.disabled = false;
        button.textContent = tr("openWrtSettings");
      }
    };
    $("refreshBtn").onclick = function () { scheduleDataRefresh(0, true, "manual"); };
    document.addEventListener("visibilitychange", function () {
      if (document.hidden) { clearTimeout(state.timer); clearStaleRetry(); }
      else startPolling();
    });
    // A bfcache entry keeps the JavaScript heap alive. Treat navigation as a
    // privacy boundary and discard the visible/live state before the document
    // can be frozen. Do not clear postLock: a router mutation may still be
    // settling and must remain serialized for this document's remaining life.
    // A restored bfcache document is reloaded, destroying every old closure and
    // then revalidating the session-only HttpOnly cookie from a clean heap.
    ...
    if (loggedOut) {
      ...
      showLogin();
      startupSessionCleanup = resumeStartupSession();
    }
  }
  document.addEventListener("DOMContentLoaded", init);
  // ===== v44 UX pack: auto day/night theme, quick search, draggable cards, speed gauge =====
  // (7) authenticated local API test with latency, jitter, payload throughput,
  // and an accessible animated gauge. This never substitutes a static asset
  // download for the protected measurement endpoint.
  speedTest = async function (button) {
    if (!beginSpeedTest(button)) return;
    var priorFocus = document.activeElement;
    var ov = document.createElement("div");
    ov.className = "smartap-ephemeral-overlay";
    ov.setAttribute("role", "dialog");
    ov.setAttribute("aria-modal", "true");
    ov.setAttribute("aria-labelledby", "sgTitle");
    ...
    patchHtml(ov, '<div style="background:var(--card,#0B1220);border:1px solid var(--border);border-radius:16px;padding:22px;text-align:center;min-width:260px">' +
      '<h3 id="sgTitle" style="margin:0 0 8px">' + esc(tr("localTest")) + '</h3>' +
      ...
      '<div id="sgVal" class="latin" aria-live="polite" style="font-size:26px;font-weight:800;margin-top:4px">0 Mbps</div>' +
      '<div id="sgSub" style="color:var(--muted);font-size:12px;margin-top:2px">&nbsp;</div>' +
      '<button id="sgClose" type="button" class="btn" style="margin-top:12px">' + (state.lang === "ar" ? "إغلاق" : "Close") + '</button></div>');
    document.body.appendChild(ov);
    var close = ov.querySelector("#sgClose");
    ...
    try {
      var result = await measureLocalSpeed(setG);
      if (!result) { closeOverlay(); return; }
      setG(result.bytesPerSecond * 8 / 1000000);
      ov.querySelector("#sgSub").textContent = state.lang === "ar"
        ? "زمن API " + fmt(result.latencyMs, 0) + " ms · تذبذب " + fmt(result.jitterMs, 0) + " ms · اختبار LAN محلي"
        : "API latency " + fmt(result.latencyMs, 0) + " ms · jitter " + fmt(result.jitterMs, 0) + " ms · local LAN test";
    } catch (e) {
      ov.querySelector("#sgSub").textContent = (state.lang === "ar" ? "فشل الاختبار: " : "Test failed: ") + e.message;
    } finally {
      finishSpeedTest(button);
    }
  };
  document.addEventListener("DOMContentLoaded", function () {
    try {
      ...
      var themePref = state.themePreference;
      function resolveTheme(p) { if (p !== "auto") return p; var h = new Date().getHours(); return (h >= 18 || h < 6) ? "dark" : "light"; }
      function applyPref(p) {
        themePref = p; state.themePreference = p;
        state.theme = resolveTheme(p);
        renderChrome();
        redrawMountedCharts($(document.body.dataset.activeSection || "overview"));
      ...
      if (navEl && navEl.parentElement && !$("navSearch")) {
        ...
        updateNavSearchCopy();
        si.oninput = function () { applyNavSearchFilter(si.value); };
      }
