...
OLD_SECURITY_RUNTIME=0
RESTORE_FAILED=0

cancel_pending_safe_apply() {
	status="$($SAFE status 2>/dev/null || true)"
	[ "$(printf '%s\n' "$status" | sed -n 's/^state=//p')" = armed ] || return 0
	token="$(printf '%s\n' "$status" | sed -n 's/^token=//p')"
	[ "${#token}" -eq 32 ] || return 1
	$SAFE confirm "$token" >/dev/null 2>&1
}
...
restore_original() {
	[ "$RESTORED" = 0 ] || return 0
	RESTORED=1
	cancel_pending_safe_apply || RESTORE_FAILED=1
	...
	if [ "$OLD_SECURITY_ENABLED" = 1 ]; then
		...
		if [ "$OLD_SECURITY_RUNTIME" = 1 ]; then restore_try /usr/sbin/cr6608-security-apply boot; else restore_try /usr/sbin/cr6608-security-apply clear-runtime; fi
	else
	...
	if [ "$OLD_FIREWALL_ENABLED" = 1 ]; then
		...
		if [ "$OLD_FIREWALL_RUNTIME" = 1 ]; then restore_try /etc/init.d/firewall start; restore_try /etc/init.d/firewall reload; else restore_try /usr/sbin/cr6608-rescue-guard firewall-stop; fi
	else
		restore_try /etc/init.d/firewall disable
		restore_try /usr/sbin/cr6608-rescue-guard firewall-stop
	fi
	...
	rm -f "$REQUEST" "$DASH_RESPONSE" "$NFT_STATE"
	if [ "$RESTORE_FAILED" = 0 ]; then
		rm -f "$BACKUP" "$BACKUP_HASHES"
	else
		printf 'restore_artifacts_preserved=%s,%s\n' "$BACKUP" "$BACKUP_HASHES" >&2
	fi
	[ "$RESTORE_FAILED" = 0 ]
}
trap restore_original EXIT
trap 'restore_original; exit 129' HUP
trap 'restore_original; exit 130' INT TERM
...
nft list table inet fw4 >/dev/null 2>&1 && OLD_FIREWALL_RUNTIME=1
nft list table bridge smartap_guard >/dev/null 2>&1 && OLD_SECURITY_RUNTIME=1
for config in $CONFIGS; do
	[ -f "/etc/config/$config" ] || {
		printf 'preflight_missing_config=%s\n' "$config" >&2
		exit 1
	}
	[ -z "$(uci -q changes "$config")" ] || {
		printf 'preflight_pending_uci=%s\n' "$config" >&2
		exit 1
	}
done
[ "$($SAFE status)" = "state=clean" ] || {
	printf 'preflight_safe_apply_not_clean\n' >&2
	exit 1
}
set --
 : >"$BACKUP_HASHES"
...
dash_post() {
	...
	printf '%s' "$body" | env \
		...
		CONTENT_LENGTH="${#body}" \
		HTTP_COOKIE="cr6608_sid_http=$SESSION_ID" \
		"$DASH" >"$DASH_RESPONSE"
...
dash_apply_mode() {
	...
	case "$country0" in ??) : ;; *) country0=US ;; esac
	case "$country1" in ??) : ;; *) country1=US ;; esac
	dash_post "section=royal&action=apply_royal&program_mode=$mode&device_ip=192.168.1.1&vlan_id=100&ssid=SmartAP-Test&ch24=11&ch5=149&country24=$country0&country5=$country1&htmode24=HE20&htmode5=HE80&security=open&hide_ssid=0&delete_previous=1&reset_button_enabled=1&reset_mode=factory&firewall_enabled=1&radio0_enabled=1&radio1_enabled=1&txpower_radio0=30&txpower_radio1=30&txpower=30"
}
