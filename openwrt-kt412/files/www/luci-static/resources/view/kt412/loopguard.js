...
return view.extend({
	...
	render: function(res){
		...
		function applyLabel(){ return [ktIc('bolt'), ' '+_('تطبيق')]; }
		var applyBtn = E('button', { class:'kt-btn', style:'margin-top:14px' }, applyLabel());
		applyBtn.onclick = function(){
			...
			call({
				...
				applyBtn.replaceChildren.apply(applyBtn, applyLabel());
				if (r && r.ok)
		...
		return E('div', {}, [
			...
			E('div', { class:'kt-grid' }, [
				field(cbStp,  [ktIc('refresh'), ' '+_('STP (شجرة الامتداد)')],
					_('يمنع حلقات اللوب على الجسر br-lan تلقائياً. يُنصح بتفعيله دائماً.')),
				field(cbIgmp, [ktIc('radio'), ' '+_('تتبّع IGMP (Multicast Snooping)')],
					_('يقلّل إغراق البث المتعدد ويحدّ من عواصف البث الناتجة عن اللوب.')),
				field(cbAp,   [ktIc('ban'), ' '+_('عزل عملاء الواي‑فاي (AP Isolation)')],
					_('يمنع أجهزة الواي‑فاي من رؤية/مهاجمة بعضها — أمان للشبكات العامة.')),
				field(cbPort, [ktIc('plug'), ' '+_('عزل المنافذ (Port Isolation)')],
