	...
	.then(function(res){
		...
		if (s && s.ok){
			...
			gauges = gauge(cpuPct, 'var(--kt-accent)', 'المعالج / الحِمل', (s.ncpu?(s.ncpu+' نواة'):''))
				...
				+ '<div class="kt-card"><h3>'+ktIcSvg('monitor')+' النظام</h3><div class="kt-kv"><span class="k">الموديل</span><span class="v">'+esc(s.model||'KT412')+'</span></div>'
				+ '<div class="kt-kv"><span class="k">الإصدار</span><span class="v">'+esc(s.release||'')+'</span></div>'
				+ '<div class="kt-kv"><span class="k">التشغيل</span><span class="v">'+ktIcSvg('timer')+' '+dur(s.uptime)+'</span></div></div>';
		}
		...
		html += '<div class="kt-grid kt-cols-2">'
			+ '<div class="kt-card"><h3>'+ktIcSvg('health')+' حالة الاتصال</h3>'+healthHtml+'</div>'
			+ '<div class="kt-card"><h3>'+ktIcSvg('link')+' سرعة الارتباط (Link)</h3>'+linksHtml+'</div></div>';
		...
		if (ms.length){
			var meshHtml = ms.map(function(m){
				var peers = (m.peers||[]).map(function(pr){ return '<div class="kt-kv"><span class="k">'+esc(pr.mac)+'</span><span class="v">'
					+ esc(pr.signal||'?')+'dBm · '+ktIcSvg('down')+''+esc(pr.rxrate||'?')+'/'+ktIcSvg('up')+''+esc(pr.txrate||'?')+' Mbps</span></div>'; }).join('')
			...
			html += '<div class="kt-card"><h3>'+ktIcSvg('mesh')+' حالة Mesh (802.11s)</h3>'+meshHtml+'</div>';
		} else {
